-- Allow Sales department to confirm payments in addition to Accounting.
-- Permission: accounting OR sales (via is_accounting_user() OR is_sales_user()).
-- Rest of the function body unchanged (guest label, invoice/booking flow, etc.).
...
DECLARE
  ...
  v_guest TEXT;
BEGIN
  IF NOT (public.is_accounting_user() OR public.is_sales_user()) THEN
    RAISE EXCEPTION 'Access denied: Only accounting or sales department can mark invoices as paid';
  END IF;
