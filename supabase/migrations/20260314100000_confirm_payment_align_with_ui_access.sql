-- Align Confirm payment authorization with app access model.
-- Allow: accounting department, sales department, super_manager role, or users with sales in category_access.
-- Fixes P0001 for users who have Sales UI access via category_access but department <> sales (e.g. facility + super_manager).

-- Ensure is_super_manager() exists (role = 'super_manager'); may already exist from migration_rls_super_manager_full_access.sql.
...
-- Helper: true if current user's profiles.category_access JSONB array contains 'sales' (same as UI Sales section).
...
AS $$
  SELECT EXISTS (
    SELECT 1 FROM public.profiles
    WHERE id = auth.uid()
    AND category_access IS NOT NULL
    AND category_access @> '["sales"]'::jsonb
  );
$$;

GRANT EXECUTE ON FUNCTION public.has_sales_category_access() TO authenticated;

-- RPC: allow accounting, sales, super_manager role, or sales category_access (no other logic changed).
