-- Fix: When manager confirms payment, booking.guest must show company name (lead_label) when
-- client_first_name/client_last_name are empty. Previously empty string '' passed IS NOT NULL
-- so we used TRIM(' ') = '' and then COALESCE('', 'Guest') = 'Guest'. Now use NULLIF(TRIM(...), '')
-- so empty name falls back to lead_label. Works for both Private (first+last) and Company (lead_label).
