-- Allow mark_invoice_paid_and_confirm_booking when invoked with service_role JWT (Vercel /api/commands/confirm-payment).
-- The HTTP command route is the trusted auth boundary (requireCommandProfile + assertCanConfirmPayment).
-- Change is only the permission gate; function body matches 20260315100000_fix_booking_company_id_in_rpc.sql.
