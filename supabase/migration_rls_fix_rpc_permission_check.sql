-- Migration: Add permission check to RPC function
-- Only accounting department should be able to execute mark_invoice_paid_and_confirm_booking
...
DECLARE
  ...
  v_end_date DATE;
BEGIN
  -- PERMISSION CHECK: Only accounting department can execute this function
  IF NOT EXISTS (
    SELECT 1 FROM public.profiles
    WHERE id = auth.uid()
    AND department = 'accounting'
  ) THEN
    RAISE EXCEPTION 'Access denied: Only accounting department can mark invoices as paid';
  END IF;
  ...
  -- This bypasses RLS because function runs as SECURITY DEFINER
  ...
  -- Step 7: Mark winning offer status = 'Accepted'
...
-- Grant execute permission (function checks department internally)
GRANT EXECUTE ON FUNCTION public.mark_invoice_paid_and_confirm_booking(UUID) TO authenticated;

-- Note: Function now has internal permission check, so we can grant to authenticated
-- but only accounting users will succeed
