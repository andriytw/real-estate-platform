...
const LOG_PREFIX = '[confirm-payment]';
...
function logStepOk(step: string, extra?: Record<string, string>): void {
  const suffix = extra && Object.keys(extra).length ? ` ${JSON.stringify(extra)}` : '';
  console.log(`${LOG_PREFIX} step=${step}:ok${suffix}`);
}

function logStepStart(step: string, extra?: Record<string, string>): void {
  const suffix = extra && Object.keys(extra).length ? ` ${JSON.stringify(extra)}` : '';
  console.log(`${LOG_PREFIX} step=${step}:start${suffix}`);
}

function logStepError(step: string, norm: ReturnType<typeof normalizeCommandError>): void {
  console.error(`${LOG_PREFIX} step=${step}:error`, {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    logStepOk('auth', { userId: profile.id });
    ...
    currentStep = 'parse-form';
    logStepStart('parse-form');
    ...
    logStepOk('parse-form', { hasPdf: String(!!pdfFile) });
    ...
    currentStep = 'invoice-lookup';
    logStepStart('invoice-lookup', { proformaId });
    await assertInvoiceExistsForConfirm(admin, proformaId);
    logStepOk('invoice-lookup');

    currentStep = 'idempotency';
    logStepStart('idempotency');
    const idem = await resolveIdempotency(admin, profile.id, 'confirm-payment', idemKey);
    if (idem.kind === 'replay') {
      logStepOk('idempotency', { kind: 'replay' });
      return json(idem.result, 200);
    }
    if (idem.kind === 'conflict') {
      logStepOk('idempotency', { kind: 'conflict' });
      return json({ error: idem.message }, 409);
    }
    logStepOk('idempotency', { kind: 'proceed' });

    try {
      let proofId: string;
      let proofHasFile = false;

      currentStep = 'proof-row';
      logStepStart('proof-row');
      if (UUID_RE.test(existingProofIdRaw)) {
        ...
        const { data: existingProof } = await withTimeout(
          admin
            .from('payment_proofs')
            .select('id, file_path, rpc_confirmed_at')
      ...
      logStepOk('proof-row', { proofId });

      if (pdfFile && !proofHasFile) {
        currentStep = 'upload-proof';
        logStepStart('upload-proof', { proofId });
        ...
        logStepOk('upload-proof');

        currentStep = 'proof-metadata-update';
        logStepStart('proof-metadata-update', { proofId });
        ...
        logStepOk('proof-metadata-update');
        proofHasFile = true;
      }

      currentStep = 'proof-rpc-state';
      logStepStart('proof-rpc-state', { proofId });
      const { data: proofState } = await withTimeout(
        admin.from('payment_proofs').select('rpc_confirmed_at').eq('id', proofId).single(),
        DB_TIMEOUT_MS,
        'check proof rpc state'
      );
      logStepOk('proof-rpc-state', { alreadyRpc: String(!!proofState?.rpc_confirmed_at) });

      let bookingId: string;

      if (proofState?.rpc_confirmed_at) {
        currentStep = 'booking-lookup';
        logStepStart('booking-lookup');
        const { data: bookingRow } = await withTimeout(
          admin
            .from('bookings')
            .select('id')
            .eq('source_invoice_id', proformaId)
            .order('created_at', { ascending: false })
            .limit(1)
            .maybeSingle(),
          DB_TIMEOUT_MS,
          'find booking for confirmed proof'
        );
        if (!bookingRow?.id) {
          throw new Error('Payment proof already confirmed but booking not found');
        }
        bookingId = bookingRow.id as string;
        logStepOk('booking-lookup', { bookingId });
      } else {
        currentStep = 'rpc-confirm';
        logStepStart('rpc-confirm', { proformaId });
        ...
        if (rpcErr) {
          ...
          if (msg.includes('already paid')) {
            logStepOk('rpc-confirm', { branch: 'already-paid' });
            currentStep = 'booking-lookup-after-paid';
            logStepStart('booking-lookup-after-paid');
            const { data: bookingRow } = await withTimeout(
              admin
                .from('bookings')
                .select('id')
                ...
                .maybeSingle(),
              DB_TIMEOUT_MS,
              'find booking after already-paid'
            );
            if (!bookingRow?.id) throw rpcErr;
            bookingId = bookingRow.id as string;
            logStepOk('booking-lookup-after-paid', { bookingId });
          } else {
            throw rpcErr;
          }
        } else if (!rpcData) {
          ...
          logStepOk('rpc-confirm', { bookingId });
        }

        currentStep = 'rpc-stamp';
        logStepStart('rpc-stamp', { proofId });
        await withTimeout(
          admin
            .from('payment_proofs')
            .update({ rpc_confirmed_at: new Date().toISOString(), updated_at: new Date().toISOString() })
            .eq('id', proofId),
          DB_TIMEOUT_MS,
          'stamp rpc_confirmed_at'
        );
        logStepOk('rpc-stamp');
      }

      if (proofHasFile) {
        currentStep = 'set-current-proof';
        logStepStart('set-current-proof', { proofId });
        await setCurrentProofAdmin(admin, proformaId, proofId);
        logStepOk('set-current-proof');
      }

      const result = { bookingId, proofId };
      await completeIdempotency(admin, idem.rowId, result);
      logStepOk('final-success', { bookingId, proofId });
      return json(result, 200);
    } catch (e) {
      const norm = normalizeCommandError(e);
      logStepError(currentStep, norm);
      if (uploadedStoragePath) {
        try {
          ...
          const cleanupNorm = normalizeCommandError(re);
          console.error(`${LOG_PREFIX} storage cleanup failed`, {
            message: cleanupNorm.message,
            code: cleanupNorm.code,
          });
        }
      }
      await failIdempotency(admin, idem.rowId, norm.message);
      return json(commandErrorResponseBody(norm, currentStep), 500);
    }
  } catch (e) {
    ...
    const norm = normalizeCommandError(e);
    logStepError(currentStep, norm);
    console.error(`${LOG_PREFIX} outer failure`, {
      message: norm.message,
      code: norm.code,
      details: norm.details,
      hint: norm.hint,
