...
const AccountDashboard: React.FC = () => {
  ...
  const [utilityKindFilter, setUtilityKindFilter] = useState<PropertyDocumentType>('supplier_electricity');
                    ...
                    {isEditingCard1 && card1Draft ? (
                        <>
                            ...
                            <div className="pb-4 border-b border-gray-700">
                                    ...
                                    <>
                                        ...
                                        <div className="mb-6">
                                            <div className="flex items-center justify-between mb-2">
                                                <span className="text-sm font-semibold text-white">Mietvertrag</span>
                                                <button type="button" onClick={() => { setNewDocType('lease_contract'); setNewDocMeta(getDefaultDocMeta('lease_contract')); setNewDocTitle(''); setNewDocDate(''); setNewDocFile(null); setAddDocumentError(null); setShowAddDocumentForm(true); }} className="p-1.5 text-emerald-500 hover:text-emerald-400 hover:bg-emerald-500/20 rounded" title="Додати"><Plus className="w-4 h-4" /></button>
                                            </div>
                                            <div className="overflow-x-auto border border-gray-700 rounded-lg">
                                                <table className="w-full text-sm min-w-[400px]">
                                                    <thead className="bg-[#23262b] text-gray-400"><tr><th className="p-2 text-left font-medium">Von</th><th className="p-2 text-left font-medium">Bis</th><th className="p-2 text-left font-medium">Nr</th><th className="p-2 text-left font-medium">Art</th><th className="p-2 w-20" /></tr></thead>
                                                    <tbody className="divide-y divide-gray-700/50">
                                                        {card1Documents.filter(d => d.type === 'lease_contract').length === 0 ? <tr><td colSpan={5} className="p-2 text-gray-500">—</td></tr> : card1Documents.filter(d => d.type === 'lease_contract').map((doc) => {
                                                            const m = (doc.meta || {}) as Record<string, unknown>;
                                                            return <tr key={doc.id} className="hover:bg-[#1C1F24]"><td className="p-2 text-white">{String(m.von ?? '—')}</td><td className="p-2 text-white">{String(m.bis ?? '—')}</td><td className="p-2 text-white">{String(m.nr ?? '—')}</td><td className="p-2 text-white">{ART_LABELS[String(m.art)] ?? String(m.art ?? '—')}</td><td className="p-2"><div className="flex items-center gap-1"><button type="button" onClick={async () => { try { const url = await propertyDocumentsService.getDocumentSignedUrl(doc.filePath); setDocPreview({ open: true, url, title: doc.title ?? DOCUMENT_TYPE_LABELS[doc.type] }); } catch (e) { alert(e instanceof Error ? e.message : 'Не вдалося відкрити'); } }} className="p-1.5 text-gray-400 hover:text-white rounded" title="Переглянути"><Eye className="w-4 h-4" /></button><button type="button" onClick={() => { if (window.confirm('Видалити документ безповоротно?')) { const pid = selectedProperty!.id; propertyDocumentsService.deletePropertyDocumentHard(doc).then(() => {}).catch((e) => alert(e?.message || 'Помилка')).finally(() => { propertyDocumentsService.listPropertyDocuments(pid).then(setCard1Documents); }); } }} className="p-1.5 text-red-400 hover:text-red-300 rounded" title="Видалити"><Trash2 className="w-4 h-4" /></button></div></td></tr>;
                                                        })}
                                                    </tbody>
                                                </table>
                                            </div>
                                        ...
                                        <div className="mb-6">
                                            <div className="flex items-center justify-between mb-2">
                                                <span className="text-sm font-semibold text-white">Übergabeprotokoll</span>
                                                <button type="button" onClick={() => { setNewDocType('handover_protocol'); setNewDocMeta(getDefaultDocMeta('handover_protocol')); setNewDocTitle(''); setNewDocDate(''); setNewDocFile(null); setAddDocumentError(null); setShowAddDocumentForm(true); }} className="p-1.5 text-emerald-500 hover:text-emerald-400 hover:bg-emerald-500/20 rounded" title="Додати"><Plus className="w-4 h-4" /></button>
                                            </div>
                                            <div className="overflow-x-auto border border-gray-700 rounded-lg">
                                                <table className="w-full text-sm min-w-[400px]">
                                                    <thead className="bg-[#23262b] text-gray-400"><tr><th className="p-2 text-left font-medium">Datum</th><th className="p-2 text-left font-medium">Nr</th><th className="p-2 text-left font-medium">Von</th><th className="p-2 text-left font-medium">An</th><th className="p-2 w-20" /></tr></thead>
                                                    <tbody className="divide-y divide-gray-700/50">
                                                        {card1Documents.filter(d => d.type === 'handover_protocol').length === 0 ? <tr><td colSpan={5} className="p-2 text-gray-500">—</td></tr> : card1Documents.filter(d => d.type === 'handover_protocol').map((doc) => {
                                                            const m = (doc.meta || {}) as Record<string, unknown>;
                                                            return <tr key={doc.id} className="hover:bg-[#1C1F24]"><td className="p-2 text-white">{String(m.datum ?? '—')}</td><td className="p-2 text-white">{String(m.nr ?? '—')}</td><td className="p-2 text-white">{String(m.von ?? '—')}</td><td className="p-2 text-white">{String(m.an ?? '—')}</td><td className="p-2"><div className="flex items-center gap-1"><button type="button" onClick={async () => { try { const url = await propertyDocumentsService.getDocumentSignedUrl(doc.filePath); setDocPreview({ open: true, url, title: doc.title ?? DOCUMENT_TYPE_LABELS[doc.type] }); } catch (e) { alert(e instanceof Error ? e.message : 'Не вдалося відкрити'); } }} className="p-1.5 text-gray-400 hover:text-white rounded" title="Переглянути"><Eye className="w-4 h-4" /></button><button type="button" onClick={() => { if (window.confirm('Видалити документ безповоротно?')) { const pid = selectedProperty!.id; propertyDocumentsService.deletePropertyDocumentHard(doc).then(() => {}).catch((e) => alert(e?.message || 'Помилка')).finally(() => { propertyDocumentsService.listPropertyDocuments(pid).then(setCard1Documents); }); } }} className="p-1.5 text-red-400 hover:text-red-300 rounded" title="Видалити"><Trash2 className="w-4 h-4" /></button></div></td></tr>;
                                                        })}
                                                    </tbody>
                                                </table>
                                            </div>
                                        ...
                                        <div className="mb-6">
                                            <div className="flex items-center justify-between mb-2">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    <select value={utilityKindFilter} onChange={e => setUtilityKindFilter(e.target.value as PropertyDocumentType)} className="bg-[#111315] border border-gray-700 rounded p-1.5 text-sm text-white">
                                                        {UTILITY_TYPES.map(t => <option key={t} value={t}>{UTILITY_KIND_LABELS[t]}</option>)}
                                                    </select>
                                                    <button type="button" onClick={() => { setNewDocType(utilityKindFilter); setNewDocMeta(getDefaultDocMeta(utilityKindFilter)); setNewDocTitle(''); setNewDocDate(''); setNewDocFile(null); setAddDocumentError(null); setShowAddDocumentForm(true); }} className="p-1.5 text-emerald-500 hover:text-emerald-400 hover:bg-emerald-500/20 rounded" title="Додати"><Plus className="w-4 h-4" /></button>
                                                </div>
                                            </div>
                                            <div className="overflow-x-auto border border-gray-700 rounded-lg">
                                                <table className="w-full text-sm min-w-[600px]">
                                                    <thead className="bg-[#23262b] text-gray-400"><tr><th className="p-2 text-left font-medium">Anb</th><th className="p-2 text-left font-medium">Firma</th><th className="p-2 text-left font-medium">Nr</th><th className="p-2 text-left font-medium">Betrag</th><th className="p-2 text-left font-medium">Fällig</th><th className="p-2 text-left font-medium">Von</th><th className="p-2 text-left font-medium">Bis</th><th className="p-2 text-left font-medium">MaLo</th><th className="p-2 w-20" /></tr></thead>
                                                    <tbody className="divide-y divide-gray-700/50">
                                                        {card1Documents.filter(d => d.type === utilityKindFilter).length === 0 ? <tr><td colSpan={9} className="p-2 text-gray-500">—</td></tr> : card1Documents.filter(d => d.type === utilityKindFilter).map((doc) => {
                                                            const m = (doc.meta || {}) as Record<string, unknown>;
                                                            return <tr key={doc.id} className="hover:bg-[#1C1F24]"><td className="p-2 text-white">{String(m.anbieter ?? '—')}</td><td className="p-2 text-white">{FIRMA_LABELS[String(m.firma)] ?? String(m.firma ?? '—')}</td><td className="p-2 text-white">{String(m.vertragsnr ?? '—')}</td><td className="p-2 text-white">{m.betrag != null ? Number(m.betrag) : '—'}</td><td className="p-2 text-white">{String(m.faellig ?? '—')}</td><td className="p-2 text-white">{String(m.von ?? '—')}</td><td className="p-2 text-white">{String(m.bis ?? '—')}</td><td className="p-2 text-white">{String(m.malo ?? '—')}</td><td className="p-2"><div className="flex items-center gap-1"><button type="button" onClick={async () => { try { const url = await propertyDocumentsService.getDocumentSignedUrl(doc.filePath); setDocPreview({ open: true, url, title: doc.title ?? DOCUMENT_TYPE_LABELS[doc.type] }); } catch (e) { alert(e instanceof Error ? e.message : 'Не вдалося відкрити'); } }} className="p-1.5 text-gray-400 hover:text-white rounded" title="Переглянути"><Eye className="w-4 h-4" /></button><button type="button" onClick={() => { if (window.confirm('Видалити документ безповоротно?')) { const pid = selectedProperty!.id; propertyDocumentsService.deletePropertyDocumentHard(doc).then(() => {}).catch((e) => alert(e?.message || 'Помилка')).finally(() => { propertyDocumentsService.listPropertyDocuments(pid).then(setCard1Documents); }); } }} className="p-1.5 text-red-400 hover:text-red-300 rounded" title="Видалити"><Trash2 className="w-4 h-4" /></button></div></td></tr>;
                                                        })}
                                                    </tbody>
                                                </table>
                                            </div>
                                        ...
                                        <div className="mb-4">
                                            <div className="flex items-center justify-between mb-2">
                                                <span className="text-sm font-semibold text-white">BK</span>
                                                <button type="button" onClick={() => { setNewDocType('bk_abrechnung'); setNewDocMeta(getDefaultDocMeta('bk_abrechnung')); setNewDocTitle(''); setNewDocDate(''); setNewDocFile(null); setAddDocumentError(null); setShowAddDocumentForm(true); }} className="p-1.5 text-emerald-500 hover:text-emerald-400 hover:bg-emerald-500/20 rounded" title="Додати"><Plus className="w-4 h-4" /></button>
                                            </div>
                                            <div className="overflow-x-auto border border-gray-700 rounded-lg">
                                                <table className="w-full text-sm min-w-[350px]">
                                                    <thead className="bg-[#23262b] text-gray-400"><tr><th className="p-2 text-left font-medium">Doc</th><th className="p-2 text-left font-medium">Von</th><th className="p-2 text-left font-medium">Bis</th><th className="p-2 text-left font-medium">Jahr</th><th className="p-2 w-20" /></tr></thead>
                                                    <tbody className="divide-y divide-gray-700/50">
                                                        {card1Documents.filter(d => d.type === 'bk_abrechnung').length === 0 ? <tr><td colSpan={5} className="p-2 text-gray-500">—</td></tr> : card1Documents.filter(d => d.type === 'bk_abrechnung').map((doc) => {
                                                            const m = (doc.meta || {}) as Record<string, unknown>;
                                                            return <tr key={doc.id} className="hover:bg-[#1C1F24]"><td className="p-2 text-white">{String(m.docDatum ?? '—')}</td><td className="p-2 text-white">{String(m.von ?? '—')}</td><td className="p-2 text-white">{String(m.bis ?? '—')}</td><td className="p-2 text-white">{String(m.jahr ?? '—')}</td><td className="p-2"><div className="flex items-center gap-1"><button type="button" onClick={async () => { try { const url = await propertyDocumentsService.getDocumentSignedUrl(doc.filePath); setDocPreview({ open: true, url, title: doc.title ?? DOCUMENT_TYPE_LABELS[doc.type] }); } catch (e) { alert(e instanceof Error ? e.message : 'Не вдалося відкрити'); } }} className="p-1.5 text-gray-400 hover:text-white rounded" title="Переглянути"><Eye className="w-4 h-4" /></button><button type="button" onClick={() => { if (window.confirm('Видалити документ безповоротно?')) { const pid = selectedProperty!.id; propertyDocumentsService.deletePropertyDocumentHard(doc).then(() => {}).catch((e) => alert(e?.message || 'Помилка')).finally(() => { propertyDocumentsService.listPropertyDocuments(pid).then(setCard1Documents); }); } }} className="p-1.5 text-red-400 hover:text-red-300 rounded" title="Видалити"><Trash2 className="w-4 h-4" /></button></div></td></tr>;
                                                        })}
                                                    </tbody>
                                                </table>
                                            </div>
                                ...
                                {showAddDocumentForm && (
                                    <div className="bg-[#111315] border border-gray-700 rounded-lg p-3 space-y-3 mt-4">
                                        <div className="text-sm font-medium text-white mb-2">Додати документ</div>
                                        {newDocType === 'lease_contract' && (
                                            <div className="grid grid-cols-2 gap-2">
                                                <div><label className="text-xs text-gray-500 block mb-1">Von *</label><input type="date" value={String(newDocMeta.von ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, von: e.target.value }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white" /></div>
                                                <div><label className="text-xs text-gray-500 block mb-1">Bis</label><input type="date" value={String(newDocMeta.bis ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, bis: e.target.value }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white" /></div>
                                                <div><label className="text-xs text-gray-500 block mb-1">Nr</label><input value={String(newDocMeta.nr ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, nr: e.target.value }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white" /></div>
                                                <div><label className="text-xs text-gray-500 block mb-1">Art *</label><select value={String(newDocMeta.art ?? 'BEFR')} onChange={e => setNewDocMeta(m => ({ ...m, art: e.target.value }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white"><option value="BEFR">Befr</option><option value="UNBEFR">Unbefr</option><option value="AUTO">Auto</option></select></div>
                                            </div>
                                        )}
                                        {newDocType === 'handover_protocol' && (
                                            <div className="grid grid-cols-2 gap-2">
                                                <div><label className="text-xs text-gray-500 block mb-1">Datum *</label><input type="date" value={String(newDocMeta.datum ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, datum: e.target.value }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white" /></div>
                                                <div><label className="text-xs text-gray-500 block mb-1">Nr</label><input value={String(newDocMeta.nr ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, nr: e.target.value }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white" /></div>
                                                <div><label className="text-xs text-gray-500 block mb-1">Von</label><input value={String(newDocMeta.von ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, von: e.target.value }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white" /></div>
                                                <div><label className="text-xs text-gray-500 block mb-1">An</label><input value={String(newDocMeta.an ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, an: e.target.value }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white" /></div>
                                            </div>
                                        )}
                                        {(newDocType === 'supplier_electricity' || newDocType === 'supplier_gas' || newDocType === 'supplier_water' || newDocType === 'supplier_waste') && (
                                            <div className="grid grid-cols-2 gap-2">
                                                <div><label className="text-xs text-gray-500 block mb-1">Kind</label><select value={newDocType} onChange={e => { const t = e.target.value as PropertyDocumentType; setNewDocType(t); setNewDocMeta(getDefaultDocMeta(t)); }} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white">{UTILITY_TYPES.map(t => <option key={t} value={t}>{UTILITY_KIND_LABELS[t]}</option>)}</select></div>
                                                <div><label className="text-xs text-gray-500 block mb-1">Firma *</label><select value={String(newDocMeta.firma ?? 'SOTISO')} onChange={e => setNewDocMeta(m => ({ ...m, firma: e.target.value }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white"><option value="SOTISO">Sotiso</option><option value="WONOVO">Wonovo</option><option value="NOWFLATS">NowFlats</option></select></div>
                                                <div><label className="text-xs text-gray-500 block mb-1">Anb</label><input value={String(newDocMeta.anbieter ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, anbieter: e.target.value }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white" /></div>
                                                <div><label className="text-xs text-gray-500 block mb-1">Nr</label><input value={String(newDocMeta.vertragsnr ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, vertragsnr: e.target.value }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white" /></div>
                                                <div><label className="text-xs text-gray-500 block mb-1">Betrag</label><input type="number" step={0.01} value={newDocMeta.betrag != null ? Number(newDocMeta.betrag) : ''} onChange={e => setNewDocMeta(m => ({ ...m, betrag: e.target.value === '' ? 0 : parseFloat(e.target.value) || 0 }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white" /></div>
                                                <div><label className="text-xs text-gray-500 block mb-1">Fällig</label><input type="date" value={String(newDocMeta.faellig ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, faellig: e.target.value }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white" /></div>
                                                <div><label className="text-xs text-gray-500 block mb-1">Von *</label><input type="date" value={String(newDocMeta.von ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, von: e.target.value }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white" /></div>
                                                <div><label className="text-xs text-gray-500 block mb-1">Bis</label><input type="date" value={String(newDocMeta.bis ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, bis: e.target.value }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white" /></div>
                                                <div className="col-span-2"><label className="text-xs text-gray-500 block mb-1">MaLo</label><input value={String(newDocMeta.malo ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, malo: e.target.value }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white" /></div>
                                            </div>
                                        )}
                                        {newDocType === 'bk_abrechnung' && (
                                            <div className="grid grid-cols-2 gap-2">
                                                <div><label className="text-xs text-gray-500 block mb-1">Doc *</label><input type="date" value={String(newDocMeta.docDatum ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, docDatum: e.target.value }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white" /></div>
                                                <div><label className="text-xs text-gray-500 block mb-1">Von</label><input type="date" value={String(newDocMeta.von ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, von: e.target.value }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white" /></div>
                                                <div><label className="text-xs text-gray-500 block mb-1">Bis</label><input type="date" value={String(newDocMeta.bis ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, bis: e.target.value }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white" /></div>
                                                <div><label className="text-xs text-gray-500 block mb-1">Jahr</label><input value={String(newDocMeta.jahr ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, jahr: e.target.value }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white" /></div>
                                            </div>
                                        )}
                                        <div><label className="text-xs text-gray-500 block mb-1">Файл *</label><input type="file" accept=".pdf,image/*" onChange={e => setNewDocFile(e.target.files?.[0] ?? null)} className="w-full text-sm text-gray-400" /></div>
                                        {addDocumentError && <p className="text-sm text-red-400">{addDocumentError}</p>}
                                        <div className="flex gap-2">
                                            <button type="button" disabled={addingDocument || !newDocFile} onClick={async () => {
                                                if (!selectedProperty || !newDocFile) return;
                                                const von = String(newDocMeta.von ?? '').trim(); const art = String(newDocMeta.art ?? '').trim();
                                                const datum = String(newDocMeta.datum ?? '').trim(); const docDatum = String(newDocMeta.docDatum ?? '').trim();
                                                if (newDocType === 'lease_contract' && (!von || !art)) { setAddDocumentError('Von та Art обовʼязкові'); return; }
                                                if (newDocType === 'handover_protocol' && !datum) { setAddDocumentError('Datum обовʼязковий'); return; }
                                                if ((newDocType === 'supplier_electricity' || newDocType === 'supplier_gas' || newDocType === 'supplier_water' || newDocType === 'supplier_waste') && !von) { setAddDocumentError('Von обовʼязковий'); return; }
                                                if (newDocType === 'bk_abrechnung' && !docDatum) { setAddDocumentError('Doc (docDatum) обовʼязковий'); return; }
                                                setAddingDocument(true); setAddDocumentError(null);
                                                const docId = crypto.randomUUID(); let filePath: string | null = null;
                                                try {
                                                    filePath = await propertyDocumentsService.uploadPropertyDocumentFile(newDocFile, selectedProperty.id, newDocType, docId);
                                                    const meta = { ...newDocMeta }; if (typeof meta.betrag === 'string') meta.betrag = parseFloat(meta.betrag as string) || 0;
                                                    await propertyDocumentsService.createPropertyDocument({ id: docId, propertyId: selectedProperty.id, type: newDocType, filePath, title: newDocTitle || null, meta });
                                                    const list = await propertyDocumentsService.listPropertyDocuments(selectedProperty.id); setCard1Documents(list);
                                                    setNewDocMeta(getDefaultDocMeta(newDocType)); setNewDocTitle(''); setNewDocDate(''); setNewDocFile(null); setShowAddDocumentForm(false);
                                                } catch (e) { if (filePath) propertyDocumentsService.removePropertyDocumentFile(filePath).catch(() => {}); setAddDocumentError(e instanceof Error ? e.message : 'Помилка'); }
                                                finally { setAddingDocument(false); }
                                            }} className="px-3 py-1.5 rounded text-sm font-medium bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white">Зберегти</button>
                                            <button type="button" onClick={() => { setShowAddDocumentForm(false); setAddDocumentError(null); setNewDocMeta({}); setNewDocFile(null); }} className="px-3 py-1.5 rounded text-sm text-gray-400 hover:text-white">Скасувати</button>
                                        </div>
                                    </div>
                                )}
                            </div>
                                ...
                                {card1DocumentsLoading ? <p className="text-sm text-gray-500">Завантаження…</p> : card1DocumentsError ? <p className="text-sm text-red-400">{card1DocumentsError}</p> : (
                                    <>
                                        <div className="mb-6">
                                            ...
                                            <div className="overflow-x-auto border border-gray-700 rounded-lg"><table className="w-full text-sm min-w-[400px]"><thead className="bg-[#23262b] text-gray-400"><tr><th className="p-2 text-left font-medium">Von</th><th className="p-2 text-left font-medium">Bis</th><th className="p-2 text-left font-medium">Nr</th><th className="p-2 text-left font-medium">Art</th><th className="p-2 w-20" /></tr></thead><tbody className="divide-y divide-gray-700/50">{card1Documents.filter(d => d.type === 'lease_contract').length === 0 ? <tr><td colSpan={5} className="p-2 text-gray-500">—</td></tr> : card1Documents.filter(d => d.type === 'lease_contract').map((doc) => { const m = (doc.meta || {}) as Record<string, unknown>; return <tr key={doc.id}><td className="p-2 text-white">{String(m.von ?? '—')}</td><td className="p-2 text-white">{String(m.bis ?? '—')}</td><td className="p-2 text-white">{String(m.nr ?? '—')}</td><td className="p-2 text-white">{ART_LABELS[String(m.art)] ?? String(m.art ?? '—')}</td><td className="p-2"><div className="flex items-center gap-1"><button type="button" onClick={async () => { try { const url = await propertyDocumentsService.getDocumentSignedUrl(doc.filePath); setDocPreview({ open: true, url, title: doc.title ?? DOCUMENT_TYPE_LABELS[doc.type] }); } catch (e) { alert(e instanceof Error ? e.message : 'Не вдалося відкрити'); } }} className="p-1.5 text-gray-400 hover:text-white rounded" title="Переглянути"><Eye className="w-4 h-4" /></button><button type="button" onClick={() => { if (window.confirm('Видалити документ безповоротно?')) { const pid = selectedProperty!.id; propertyDocumentsService.deletePropertyDocumentHard(doc).then(() => {}).catch((e) => alert(e?.message || 'Помилка')).finally(() => { propertyDocumentsService.listPropertyDocuments(pid).then(setCard1Documents); }); } }} className="p-1.5 text-red-400 hover:text-red-300 rounded" title="Видалити"><Trash2 className="w-4 h-4" /></button></div></td></tr>; })}</tbody></table></div>
                                        </div>
                                        <div className="mb-6">
                                            <div className="flex items-center justify-between mb-2"><span className="text-sm font-semibold text-white">Übergabeprotokoll</span><button type="button" onClick={() => { setNewDocType('handover_protocol'); setNewDocMeta(getDefaultDocMeta('handover_protocol')); setNewDocTitle(''); setNewDocDate(''); setNewDocFile(null); setAddDocumentError(null); setShowAddDocumentForm(true); }} className="p-1.5 text-emerald-500 hover:text-emerald-400 hover:bg-emerald-500/20 rounded" title="Додати"><Plus className="w-4 h-4" /></button></div>
                                            <div className="overflow-x-auto border border-gray-700 rounded-lg"><table className="w-full text-sm min-w-[400px]"><thead className="bg-[#23262b] text-gray-400"><tr><th className="p-2 text-left font-medium">Datum</th><th className="p-2 text-left font-medium">Nr</th><th className="p-2 text-left font-medium">Von</th><th className="p-2 text-left font-medium">An</th><th className="p-2 w-20" /></tr></thead><tbody className="divide-y divide-gray-700/50">{card1Documents.filter(d => d.type === 'handover_protocol').length === 0 ? <tr><td colSpan={5} className="p-2 text-gray-500">—</td></tr> : card1Documents.filter(d => d.type === 'handover_protocol').map((doc) => { const m = (doc.meta || {}) as Record<string, unknown>; return <tr key={doc.id}><td className="p-2 text-white">{String(m.datum ?? '—')}</td><td className="p-2 text-white">{String(m.nr ?? '—')}</td><td className="p-2 text-white">{String(m.von ?? '—')}</td><td className="p-2 text-white">{String(m.an ?? '—')}</td><td className="p-2"><div className="flex items-center gap-1"><button type="button" onClick={async () => { try { const url = await propertyDocumentsService.getDocumentSignedUrl(doc.filePath); setDocPreview({ open: true, url, title: doc.title ?? DOCUMENT_TYPE_LABELS[doc.type] }); } catch (e) { alert(e instanceof Error ? e.message : 'Не вдалося відкрити'); } }} className="p-1.5 text-gray-400 hover:text-white rounded" title="Переглянути"><Eye className="w-4 h-4" /></button><button type="button" onClick={() => { if (window.confirm('Видалити документ безповоротно?')) { const pid = selectedProperty!.id; propertyDocumentsService.deletePropertyDocumentHard(doc).then(() => {}).catch((e) => alert(e?.message || 'Помилка')).finally(() => { propertyDocumentsService.listPropertyDocuments(pid).then(setCard1Documents); }); } }} className="p-1.5 text-red-400 hover:text-red-300 rounded" title="Видалити"><Trash2 className="w-4 h-4" /></button></div></td></tr>; })}</tbody></table></div>
                                        </div>
                                        <div className="mb-6">
                                            <div className="flex items-center justify-between mb-2"><span className="text-sm font-semibold text-white">Utility</span><div className="flex items-center gap-2"><select value={utilityKindFilter} onChange={e => setUtilityKindFilter(e.target.value as PropertyDocumentType)} className="bg-[#111315] border border-gray-700 rounded p-1.5 text-sm text-white">{UTILITY_TYPES.map(t => <option key={t} value={t}>{UTILITY_KIND_LABELS[t]}</option>)}</select><button type="button" onClick={() => { setNewDocType(utilityKindFilter); setNewDocMeta(getDefaultDocMeta(utilityKindFilter)); setNewDocTitle(''); setNewDocDate(''); setNewDocFile(null); setAddDocumentError(null); setShowAddDocumentForm(true); }} className="p-1.5 text-emerald-500 hover:text-emerald-400 hover:bg-emerald-500/20 rounded" title="Додати"><Plus className="w-4 h-4" /></button></div></div>
                                            <div className="overflow-x-auto border border-gray-700 rounded-lg"><table className="w-full text-sm min-w-[600px]"><thead className="bg-[#23262b] text-gray-400"><tr><th className="p-2 text-left font-medium">Anb</th><th className="p-2 text-left font-medium">Firma</th><th className="p-2 text-left font-medium">Nr</th><th className="p-2 text-left font-medium">Betrag</th><th className="p-2 text-left font-medium">Fällig</th><th className="p-2 text-left font-medium">Von</th><th className="p-2 text-left font-medium">Bis</th><th className="p-2 text-left font-medium">MaLo</th><th className="p-2 w-20" /></tr></thead><tbody className="divide-y divide-gray-700/50">{card1Documents.filter(d => d.type === utilityKindFilter).length === 0 ? <tr><td colSpan={9} className="p-2 text-gray-500">—</td></tr> : card1Documents.filter(d => d.type === utilityKindFilter).map((doc) => { const m = (doc.meta || {}) as Record<string, unknown>; return <tr key={doc.id}><td className="p-2 text-white">{String(m.anbieter ?? '—')}</td><td className="p-2 text-white">{FIRMA_LABELS[String(m.firma)] ?? String(m.firma ?? '—')}</td><td className="p-2 text-white">{String(m.vertragsnr ?? '—')}</td><td className="p-2 text-white">{m.betrag != null ? Number(m.betrag) : '—'}</td><td className="p-2 text-white">{String(m.faellig ?? '—')}</td><td className="p-2 text-white">{String(m.von ?? '—')}</td><td className="p-2 text-white">{String(m.bis ?? '—')}</td><td className="p-2 text-white">{String(m.malo ?? '—')}</td><td className="p-2"><div className="flex items-center gap-1"><button type="button" onClick={async () => { try { const url = await propertyDocumentsService.getDocumentSignedUrl(doc.filePath); setDocPreview({ open: true, url, title: doc.title ?? DOCUMENT_TYPE_LABELS[doc.type] }); } catch (e) { alert(e instanceof Error ? e.message : 'Не вдалося відкрити'); } }} className="p-1.5 text-gray-400 hover:text-white rounded" title="Переглянути"><Eye className="w-4 h-4" /></button><button type="button" onClick={() => { if (window.confirm('Видалити документ безповоротно?')) { const pid = selectedProperty!.id; propertyDocumentsService.deletePropertyDocumentHard(doc).then(() => {}).catch((e) => alert(e?.message || 'Помилка')).finally(() => { propertyDocumentsService.listPropertyDocuments(pid).then(setCard1Documents); }); } }} className="p-1.5 text-red-400 hover:text-red-300 rounded" title="Видалити"><Trash2 className="w-4 h-4" /></button></div></td></tr>; })}</tbody></table></div>
                                        </div>
                                        <div className="mb-4">
                                            <div className="flex items-center justify-between mb-2"><span className="text-sm font-semibold text-white">BK</span><button type="button" onClick={() => { setNewDocType('bk_abrechnung'); setNewDocMeta(getDefaultDocMeta('bk_abrechnung')); setNewDocTitle(''); setNewDocDate(''); setNewDocFile(null); setAddDocumentError(null); setShowAddDocumentForm(true); }} className="p-1.5 text-emerald-500 hover:text-emerald-400 hover:bg-emerald-500/20 rounded" title="Додати"><Plus className="w-4 h-4" /></button></div>
                                            <div className="overflow-x-auto border border-gray-700 rounded-lg"><table className="w-full text-sm min-w-[350px]"><thead className="bg-[#23262b] text-gray-400"><tr><th className="p-2 text-left font-medium">Doc</th><th className="p-2 text-left font-medium">Von</th><th className="p-2 text-left font-medium">Bis</th><th className="p-2 text-left font-medium">Jahr</th><th className="p-2 w-20" /></tr></thead><tbody className="divide-y divide-gray-700/50">{card1Documents.filter(d => d.type === 'bk_abrechnung').length === 0 ? <tr><td colSpan={5} className="p-2 text-gray-500">—</td></tr> : card1Documents.filter(d => d.type === 'bk_abrechnung').map((doc) => { const m = (doc.meta || {}) as Record<string, unknown>; return <tr key={doc.id}><td className="p-2 text-white">{String(m.docDatum ?? '—')}</td><td className="p-2 text-white">{String(m.von ?? '—')}</td><td className="p-2 text-white">{String(m.bis ?? '—')}</td><td className="p-2 text-white">{String(m.jahr ?? '—')}</td><td className="p-2"><div className="flex items-center gap-1"><button type="button" onClick={async () => { try { const url = await propertyDocumentsService.getDocumentSignedUrl(doc.filePath); setDocPreview({ open: true, url, title: doc.title ?? DOCUMENT_TYPE_LABELS[doc.type] }); } catch (e) { alert(e instanceof Error ? e.message : 'Не вдалося відкрити'); } }} className="p-1.5 text-gray-400 hover:text-white rounded" title="Переглянути"><Eye className="w-4 h-4" /></button><button type="button" onClick={() => { if (window.confirm('Видалити документ безповоротно?')) { const pid = selectedProperty!.id; propertyDocumentsService.deletePropertyDocumentHard(doc).then(() => {}).catch((e) => alert(e?.message || 'Помилка')).finally(() => { propertyDocumentsService.listPropertyDocuments(pid).then(setCard1Documents); }); } }} className="p-1.5 text-red-400 hover:text-red-300 rounded" title="Видалити"><Trash2 className="w-4 h-4" /></button></div></td></tr>; })}</tbody></table></div>
                                        </div>
                                    </>
                                )}
                                {showAddDocumentForm && (
                                    <div className="bg-[#111315] border border-gray-700 rounded-lg p-3 space-y-3 mt-4">
                                        <div className="text-sm font-medium text-white mb-2">Додати документ</div>
                                        {newDocType === 'lease_contract' && (<div className="grid grid-cols-2 gap-2"><div><label className="text-xs text-gray-500 block mb-1">Von *</label><input type="date" value={String(newDocMeta.von ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, von: e.target.value }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white" /></div><div><label className="text-xs text-gray-500 block mb-1">Bis</label><input type="date" value={String(newDocMeta.bis ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, bis: e.target.value }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white" /></div><div><label className="text-xs text-gray-500 block mb-1">Nr</label><input value={String(newDocMeta.nr ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, nr: e.target.value }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white" /></div><div><label className="text-xs text-gray-500 block mb-1">Art *</label><select value={String(newDocMeta.art ?? 'BEFR')} onChange={e => setNewDocMeta(m => ({ ...m, art: e.target.value }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white"><option value="BEFR">Befr</option><option value="UNBEFR">Unbefr</option><option value="AUTO">Auto</option></select></div></div>)}
                                        {newDocType === 'handover_protocol' && (<div className="grid grid-cols-2 gap-2"><div><label className="text-xs text-gray-500 block mb-1">Datum *</label><input type="date" value={String(newDocMeta.datum ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, datum: e.target.value }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white" /></div><div><label className="text-xs text-gray-500 block mb-1">Nr</label><input value={String(newDocMeta.nr ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, nr: e.target.value }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white" /></div><div><label className="text-xs text-gray-500 block mb-1">Von</label><input value={String(newDocMeta.von ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, von: e.target.value }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white" /></div><div><label className="text-xs text-gray-500 block mb-1">An</label><input value={String(newDocMeta.an ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, an: e.target.value }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white" /></div></div>)}
                                        {(newDocType === 'supplier_electricity' || newDocType === 'supplier_gas' || newDocType === 'supplier_water' || newDocType === 'supplier_waste') && (<div className="grid grid-cols-2 gap-2"><div><label className="text-xs text-gray-500 block mb-1">Kind</label><select value={newDocType} onChange={e => { const t = e.target.value as PropertyDocumentType; setNewDocType(t); setNewDocMeta(getDefaultDocMeta(t)); }} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white">{UTILITY_TYPES.map(t => <option key={t} value={t}>{UTILITY_KIND_LABELS[t]}</option>)}</select></div><div><label className="text-xs text-gray-500 block mb-1">Firma *</label><select value={String(newDocMeta.firma ?? 'SOTISO')} onChange={e => setNewDocMeta(m => ({ ...m, firma: e.target.value }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white"><option value="SOTISO">Sotiso</option><option value="WONOVO">Wonovo</option><option value="NOWFLATS">NowFlats</option></select></div><div><label className="text-xs text-gray-500 block mb-1">Anb</label><input value={String(newDocMeta.anbieter ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, anbieter: e.target.value }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white" /></div><div><label className="text-xs text-gray-500 block mb-1">Nr</label><input value={String(newDocMeta.vertragsnr ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, vertragsnr: e.target.value }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white" /></div><div><label className="text-xs text-gray-500 block mb-1">Betrag</label><input type="number" step={0.01} value={newDocMeta.betrag != null ? Number(newDocMeta.betrag) : ''} onChange={e => setNewDocMeta(m => ({ ...m, betrag: e.target.value === '' ? 0 : parseFloat(e.target.value) || 0 }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white" /></div><div><label className="text-xs text-gray-500 block mb-1">Fällig</label><input type="date" value={String(newDocMeta.faellig ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, faellig: e.target.value }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white" /></div><div><label className="text-xs text-gray-500 block mb-1">Von *</label><input type="date" value={String(newDocMeta.von ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, von: e.target.value }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white" /></div><div><label className="text-xs text-gray-500 block mb-1">Bis</label><input type="date" value={String(newDocMeta.bis ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, bis: e.target.value }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white" /></div><div className="col-span-2"><label className="text-xs text-gray-500 block mb-1">MaLo</label><input value={String(newDocMeta.malo ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, malo: e.target.value }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white" /></div></div>)}
                                        {newDocType === 'bk_abrechnung' && (<div className="grid grid-cols-2 gap-2"><div><label className="text-xs text-gray-500 block mb-1">Doc *</label><input type="date" value={String(newDocMeta.docDatum ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, docDatum: e.target.value }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white" /></div><div><label className="text-xs text-gray-500 block mb-1">Von</label><input type="date" value={String(newDocMeta.von ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, von: e.target.value }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white" /></div><div><label className="text-xs text-gray-500 block mb-1">Bis</label><input type="date" value={String(newDocMeta.bis ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, bis: e.target.value }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white" /></div><div><label className="text-xs text-gray-500 block mb-1">Jahr</label><input value={String(newDocMeta.jahr ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, jahr: e.target.value }))} className="w-full bg-[#0D1117] border border-gray-700 rounded p-2 text-sm text-white" /></div></div>)}
                                        <div><label className="text-xs text-gray-500 block mb-1">Файл *</label><input type="file" accept=".pdf,image/*" onChange={e => setNewDocFile(e.target.files?.[0] ?? null)} className="w-full text-sm text-gray-400" /></div>
                                        {addDocumentError && <p className="text-sm text-red-400">{addDocumentError}</p>}
                                        <div className="flex gap-2"><button type="button" disabled={addingDocument || !newDocFile} onClick={async () => { if (!selectedProperty || !newDocFile) return; const von = String(newDocMeta.von ?? '').trim(); const art = String(newDocMeta.art ?? '').trim(); const datum = String(newDocMeta.datum ?? '').trim(); const docDatum = String(newDocMeta.docDatum ?? '').trim(); if (newDocType === 'lease_contract' && (!von || !art)) { setAddDocumentError('Von та Art обовʼязкові'); return; } if (newDocType === 'handover_protocol' && !datum) { setAddDocumentError('Datum обовʼязковий'); return; } if ((newDocType === 'supplier_electricity' || newDocType === 'supplier_gas' || newDocType === 'supplier_water' || newDocType === 'supplier_waste') && !von) { setAddDocumentError('Von обовʼязковий'); return; } if (newDocType === 'bk_abrechnung' && !docDatum) { setAddDocumentError('Doc обовʼязковий'); return; } setAddingDocument(true); setAddDocumentError(null); const docId = crypto.randomUUID(); let filePath: string | null = null; try { filePath = await propertyDocumentsService.uploadPropertyDocumentFile(newDocFile, selectedProperty.id, newDocType, docId); const meta = { ...newDocMeta }; if (typeof meta.betrag === 'string') meta.betrag = parseFloat(meta.betrag as string) || 0; await propertyDocumentsService.createPropertyDocument({ id: docId, propertyId: selectedProperty.id, type: newDocType, filePath, title: newDocTitle || null, meta }); const list = await propertyDocumentsService.listPropertyDocuments(selectedProperty.id); setCard1Documents(list); setNewDocMeta(getDefaultDocMeta(newDocType)); setNewDocTitle(''); setNewDocDate(''); setNewDocFile(null); setShowAddDocumentForm(false); } catch (e) { if (filePath) propertyDocumentsService.removePropertyDocumentFile(filePath).catch(() => {}); setAddDocumentError(e instanceof Error ? e.message : 'Помилка'); } finally { setAddingDocument(false); } }} className="px-3 py-1.5 rounded text-sm font-medium bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white">Зберегти</button><button type="button" onClick={() => { setShowAddDocumentForm(false); setAddDocumentError(null); setNewDocMeta({}); setNewDocFile(null); }} className="px-3 py-1.5 rounded text-sm text-gray-400 hover:text-white">Скасувати</button></div>
                                    </div>
                                )}
                            </div>
                        </>
