                                    ...
                                    <>
                                        ...
                                        <div className="mb-4">
                                            ...
                                            <div className="overflow-x-auto border border-gray-700 rounded-lg">
                                                <table className="w-full text-xs min-w-[360px] table-fixed">
                                                    <thead className="bg-[#23262b] text-gray-400"><tr><th className="px-2 py-1 text-left font-medium w-24">Von</th><th className="px-2 py-1 text-left font-medium w-24">Bis</th><th className="px-2 py-1 text-left font-medium w-16">Nr</th><th className="px-2 py-1 text-left font-medium w-16">Art</th><th className="px-2 py-1 w-20" /></tr></thead>
                                                    <tbody className="divide-y divide-gray-700/50">
                                                        {showAddDocumentForm && newDocType === 'lease_contract' && (
                                                            <tr className="bg-[#0D1117]/80">
                                                                <td className="px-2 py-1"><input type="date" value={String(newDocMeta.von ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, von: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td>
                                                                <td className="px-2 py-1"><input type="date" value={String(newDocMeta.bis ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, bis: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td>
                                                                <td className="px-2 py-1"><input value={String(newDocMeta.nr ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, nr: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td>
                                                                <td className="px-2 py-1"><select value={String(newDocMeta.art ?? 'BEFR')} onChange={e => setNewDocMeta(m => ({ ...m, art: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white"><option value="BEFR">Befr</option><option value="UNBEFR">Unbefr</option><option value="AUTO">Auto</option></select></td>
                                                                <td className="px-2 py-1"><div className="flex items-center gap-0.5 flex-nowrap"><button type="button" onClick={() => addDocumentFileInputRef.current?.click()} className="p-1 text-gray-400 hover:text-white rounded" title="Файл"><Paperclip className="w-3.5 h-3.5" /></button><span className="truncate max-w-16 text-gray-500" title={newDocFile?.name}>{newDocFile?.name ?? '—'}</span><button type="button" disabled={addingDocument || !newDocFile} onClick={async () => { if (!selectedProperty || !newDocFile) return; const von = String(newDocMeta.von ?? '').trim(); const art = String(newDocMeta.art ?? '').trim(); if (!von || !art) { setAddDocumentError('Von та Art обовʼязкові'); return; } setAddingDocument(true); setAddDocumentError(null); const docId = crypto.randomUUID(); let filePath: string | null = null; try { filePath = await propertyDocumentsService.uploadPropertyDocumentFile(newDocFile, selectedProperty.id, 'lease_contract', docId); const meta = { ...newDocMeta }; await propertyDocumentsService.createPropertyDocument({ id: docId, propertyId: selectedProperty.id, type: 'lease_contract', filePath, title: null, meta }); const list = await propertyDocumentsService.listPropertyDocuments(selectedProperty.id); setCard1Documents(list); setNewDocMeta(getDefaultDocMeta('lease_contract')); setNewDocFile(null); setShowAddDocumentForm(false); if (addDocumentFileInputRef.current) addDocumentFileInputRef.current.value = ''; } catch (e) { if (filePath) propertyDocumentsService.removePropertyDocumentFile(filePath).catch(() => {}); setAddDocumentError(e instanceof Error ? e.message : 'Помилка'); } finally { setAddingDocument(false); } }} className="p-1 text-emerald-500 hover:text-emerald-400 rounded" title="Зберегти"><Check className="w-3.5 h-3.5" /></button><button type="button" onClick={() => { setShowAddDocumentForm(false); setAddDocumentError(null); setNewDocMeta({}); setNewDocFile(null); if (addDocumentFileInputRef.current) addDocumentFileInputRef.current.value = ''; }} className="p-1 text-gray-400 hover:text-white rounded" title="Скасувати"><X className="w-3.5 h-3.5" /></button></div></td></tr>
                                                        )}
                                                        {showAddDocumentForm && newDocType === 'lease_contract' && addDocumentError && <tr><td colSpan={5} className="px-2 py-0.5 text-red-400 text-xs">{addDocumentError}</td></tr>}
                                                        {card1Documents.filter(d => d.type === 'lease_contract').length === 0 && !(showAddDocumentForm && newDocType === 'lease_contract') ? <tr><td colSpan={5} className="px-2 py-1 text-gray-500 text-xs whitespace-nowrap">Keine Einträge</td></tr> : card1Documents.filter(d => d.type === 'lease_contract').map((doc) => {
                                                            const m = (doc.meta || {}) as Record<string, unknown>;
                                                            return <tr key={doc.id} className="hover:bg-[#1C1F24]"><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.von ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.bis ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.nr ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{ART_LABELS[String(m.art)] ?? String(m.art ?? '—')}</td><td className="px-2 py-1"><div className="flex items-center gap-0.5 justify-end"><button type="button" onClick={async () => { try { const url = await propertyDocumentsService.getDocumentSignedUrl(doc.filePath); setDocPreview({ open: true, url, title: doc.title ?? DOCUMENT_TYPE_LABELS[doc.type] }); } catch (e) { alert(e instanceof Error ? e.message : 'Не вдалося відкрити'); } }} className="p-1 text-gray-400 hover:text-white rounded" title="Переглянути"><Eye className="w-4 h-4" /></button><button type="button" onClick={() => { if (window.confirm('Видалити документ безповоротно?')) { const pid = selectedProperty!.id; propertyDocumentsService.deletePropertyDocumentHard(doc).then(() => {}).catch((e) => alert(e?.message || 'Помилка')).finally(() => { propertyDocumentsService.listPropertyDocuments(pid).then(setCard1Documents); }); } }} className="p-1 text-red-400 hover:text-red-300 rounded" title="Видалити"><Trash2 className="w-4 h-4" /></button></div></td></tr>;
                                                        })}
                                                    </tbody>
                                                </table>
                                            </div>
                                        ...
                                        <div className="mb-4">
                                            ...
                                            <div className="overflow-x-auto border border-gray-700 rounded-lg">
                                                <table className="w-full text-xs min-w-[360px] table-fixed">
                                                    <thead className="bg-[#23262b] text-gray-400"><tr><th className="px-2 py-1 text-left font-medium w-24">Datum</th><th className="px-2 py-1 text-left font-medium w-16">Nr</th><th className="px-2 py-1 text-left font-medium w-24">Von</th><th className="px-2 py-1 text-left font-medium w-16">An</th><th className="px-2 py-1 w-20" /></tr></thead>
                                                    <tbody className="divide-y divide-gray-700/50">
                                                        {showAddDocumentForm && newDocType === 'handover_protocol' && (
                                                            <tr className="bg-[#0D1117]/80">
                                                                <td className="px-2 py-1"><input type="date" value={String(newDocMeta.datum ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, datum: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td>
                                                                <td className="px-2 py-1"><input value={String(newDocMeta.nr ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, nr: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td>
                                                                <td className="px-2 py-1"><select value={String(newDocMeta.vonId ?? '')} onChange={e => { const id = e.target.value; const entry = addressBookEntries.find(x => x.id === id); setNewDocMeta(m => ({ ...m, vonId: id, vonName: entry?.name ?? '' })); }} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white"><option value="">—</option>{addressBookEntries.map(e => <option key={e.id ?? e.name} value={e.id ?? ''}>{e.name}</option>)}</select></td>
                                                                <td className="px-2 py-1"><select value={String(newDocMeta.anId ?? '')} onChange={e => { const id = e.target.value; const entry = addressBookEntries.find(x => x.id === id); setNewDocMeta(m => ({ ...m, anId: id, anName: entry?.name ?? '' })); }} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white"><option value="">—</option>{addressBookEntries.map(e => <option key={e.id ?? e.name} value={e.id ?? ''}>{e.name}</option>)}</select></td>
                                                        ...
                                                        {showAddDocumentForm && newDocType === 'handover_protocol' && addDocumentError && <tr><td colSpan={5} className="px-2 py-0.5 text-red-400 text-xs">{addDocumentError}</td></tr>}
                                                        {card1Documents.filter(d => d.type === 'handover_protocol').length === 0 && !(showAddDocumentForm && newDocType === 'handover_protocol') ? <tr><td colSpan={5} className="px-2 py-1 text-gray-500 text-xs whitespace-nowrap">Keine Einträge</td></tr> : card1Documents.filter(d => d.type === 'handover_protocol').map((doc) => {
                                                            const m = (doc.meta || {}) as Record<string, unknown>;
                                                            ...
                                                            return <tr key={doc.id} className="hover:bg-[#1C1F24]"><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.datum ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.nr ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{vonDisplay}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{anDisplay}</td><td className="px-2 py-1"><div className="flex items-center gap-0.5 justify-end"><button type="button" onClick={async () => { try { const url = await propertyDocumentsService.getDocumentSignedUrl(doc.filePath); setDocPreview({ open: true, url, title: doc.title ?? DOCUMENT_TYPE_LABELS[doc.type] }); } catch (e) { alert(e instanceof Error ? e.message : 'Не вдалося відкрити'); } }} className="p-1 text-gray-400 hover:text-white rounded" title="Переглянути"><Eye className="w-4 h-4" /></button><button type="button" onClick={() => { if (window.confirm('Видалити документ безповоротно?')) { const pid = selectedProperty!.id; propertyDocumentsService.deletePropertyDocumentHard(doc).then(() => {}).catch((e) => alert(e?.message || 'Помилка')).finally(() => { propertyDocumentsService.listPropertyDocuments(pid).then(setCard1Documents); }); } }} className="p-1 text-red-400 hover:text-red-300 rounded" title="Видалити"><Trash2 className="w-4 h-4" /></button></div></td></tr>;
                                                        })}
                                                    </tbody>
                                                </table>
                                            </div>
                                        ...
                                        <div className="mb-4">
                                            ...
                                            <div className="overflow-x-auto border border-gray-700 rounded-lg">
                                                <table className="w-full text-xs min-w-[680px] table-fixed">
                                                    <thead className="bg-[#23262b] text-gray-400"><tr><th className="px-2 py-1 text-left font-medium min-w-[120px] w-28">Kind</th><th className="px-2 py-1 text-left font-medium w-20">Anb</th><th className="px-2 py-1 text-left font-medium w-16">Firma</th><th className="px-2 py-1 text-left font-medium w-14">Nr</th><th className="px-2 py-1 text-left font-medium w-14">Betrag</th><th className="px-2 py-1 text-left font-medium w-20">Fällig</th><th className="px-2 py-1 text-left font-medium w-20">Von</th><th className="px-2 py-1 text-left font-medium w-20">Bis</th><th className="px-2 py-1 text-left font-medium w-16">MaLo</th><th className="px-2 py-1 w-20" /></tr></thead>
                                                    <tbody className="divide-y divide-gray-700/50">
                                                        {showAddDocumentForm && (newDocType === 'supplier_electricity' || newDocType === 'supplier_gas' || newDocType === 'supplier_water' || newDocType === 'supplier_waste') && (
                                                            <tr className="bg-[#0D1117]/80">
                                                                <td className="px-2 py-1"><select value={newDocType} onChange={e => { const t = e.target.value as PropertyDocumentType; setNewDocType(t); setNewDocMeta(getDefaultDocMeta(t)); }} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white">{UTILITY_TYPES.map(t => <option key={t} value={t}>{UTILITY_KIND_LABELS[t]}</option>)}</select></td>
                                                                <td className="px-2 py-1"><input value={String(newDocMeta.anbieter ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, anbieter: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td>
                                                                <td className="px-2 py-1"><select value={String(newDocMeta.firma ?? 'SOTISO')} onChange={e => setNewDocMeta(m => ({ ...m, firma: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white"><option value="SOTISO">Sotiso</option><option value="WONOVO">Wonovo</option><option value="NOWFLATS">NowFlats</option></select></td>
                                                                <td className="px-2 py-1"><input value={String(newDocMeta.vertragsnr ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, vertragsnr: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td>
                                                                <td className="px-2 py-1"><input type="number" step={0.01} value={newDocMeta.betrag != null ? Number(newDocMeta.betrag) : ''} onChange={e => setNewDocMeta(m => ({ ...m, betrag: e.target.value === '' ? 0 : parseFloat(e.target.value) || 0 }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td>
                                                                <td className="px-2 py-1"><input type="date" value={String(newDocMeta.faellig ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, faellig: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td>
                                                                <td className="px-2 py-1"><input type="date" value={String(newDocMeta.von ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, von: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td>
                                                                <td className="px-2 py-1"><input type="date" value={String(newDocMeta.bis ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, bis: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td>
                                                                <td className="px-2 py-1"><input value={String(newDocMeta.malo ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, malo: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td>
                                                        ...
                                                        {showAddDocumentForm && (newDocType === 'supplier_electricity' || newDocType === 'supplier_gas' || newDocType === 'supplier_water' || newDocType === 'supplier_waste') && addDocumentError && <tr><td colSpan={10} className="px-2 py-0.5 text-red-400 text-xs">{addDocumentError}</td></tr>}
                                                        {card1Documents.filter(d => UTILITY_TYPES.includes(d.type)).length === 0 && !(showAddDocumentForm && (newDocType === 'supplier_electricity' || newDocType === 'supplier_gas' || newDocType === 'supplier_water' || newDocType === 'supplier_waste')) ? <tr><td colSpan={10} className="px-2 py-1 text-gray-500 text-xs whitespace-nowrap">Keine Einträge</td></tr> : card1Documents.filter(d => UTILITY_TYPES.includes(d.type)).map((doc) => {
                                                            const m = (doc.meta || {}) as Record<string, unknown>;
                                                            return <tr key={doc.id} className="hover:bg-[#1C1F24]"><td className="px-2 py-1 text-white truncate whitespace-nowrap">{UTILITY_KIND_LABELS[doc.type] ?? doc.type}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.anbieter ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{FIRMA_LABELS[String(m.firma)] ?? String(m.firma ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.vertragsnr ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{m.betrag != null ? Number(m.betrag) : '—'}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.faellig ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.von ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.bis ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.malo ?? '—')}</td><td className="px-2 py-1"><div className="flex items-center gap-0.5 justify-end"><button type="button" onClick={async () => { try { const url = await propertyDocumentsService.getDocumentSignedUrl(doc.filePath); setDocPreview({ open: true, url, title: doc.title ?? DOCUMENT_TYPE_LABELS[doc.type] }); } catch (e) { alert(e instanceof Error ? e.message : 'Не вдалося відкрити'); } }} className="p-1 text-gray-400 hover:text-white rounded" title="Переглянути"><Eye className="w-4 h-4" /></button><button type="button" onClick={() => { if (window.confirm('Видалити документ безповоротно?')) { const pid = selectedProperty!.id; propertyDocumentsService.deletePropertyDocumentHard(doc).then(() => {}).catch((e) => alert(e?.message || 'Помилка')).finally(() => { propertyDocumentsService.listPropertyDocuments(pid).then(setCard1Documents); }); } }} className="p-1 text-red-400 hover:text-red-300 rounded" title="Видалити"><Trash2 className="w-4 h-4" /></button></div></td></tr>;
                                                        })}
                                                    </tbody>
                                                </table>
                                            </div>
                                        ...
                                        <div className="mb-4">
                                            ...
                                            <div className="overflow-x-auto border border-gray-700 rounded-lg">
                                                <table className="w-full text-xs min-w-[320px] table-fixed">
                                                    <thead className="bg-[#23262b] text-gray-400"><tr><th className="px-2 py-1 text-left font-medium w-24">Doc</th><th className="px-2 py-1 text-left font-medium w-24">Von</th><th className="px-2 py-1 text-left font-medium w-24">Bis</th><th className="px-2 py-1 text-left font-medium w-14">Jahr</th><th className="px-2 py-1 w-20" /></tr></thead>
                                                    <tbody className="divide-y divide-gray-700/50">
                                                        {showAddDocumentForm && newDocType === 'bk_abrechnung' && (
                                                            <tr className="bg-[#0D1117]/80">
                                                                <td className="px-2 py-1"><input type="date" value={String(newDocMeta.docDatum ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, docDatum: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td>
                                                                <td className="px-2 py-1"><input type="date" value={String(newDocMeta.von ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, von: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td>
                                                                <td className="px-2 py-1"><input type="date" value={String(newDocMeta.bis ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, bis: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td>
                                                                <td className="px-2 py-1"><input value={String(newDocMeta.jahr ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, jahr: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td>
                                                        ...
                                                        {showAddDocumentForm && newDocType === 'bk_abrechnung' && addDocumentError && <tr><td colSpan={5} className="px-2 py-0.5 text-red-400 text-xs">{addDocumentError}</td></tr>}
                                                        {card1Documents.filter(d => d.type === 'bk_abrechnung').length === 0 && !(showAddDocumentForm && newDocType === 'bk_abrechnung') ? <tr><td colSpan={5} className="px-2 py-1 text-gray-500 text-xs whitespace-nowrap">Keine Einträge</td></tr> : card1Documents.filter(d => d.type === 'bk_abrechnung').map((doc) => {
                                                            const m = (doc.meta || {}) as Record<string, unknown>;
                                                            return <tr key={doc.id} className="hover:bg-[#1C1F24]"><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.docDatum ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.von ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.bis ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.jahr ?? '—')}</td><td className="px-2 py-1"><div className="flex items-center gap-0.5 justify-end"><button type="button" onClick={async () => { try { const url = await propertyDocumentsService.getDocumentSignedUrl(doc.filePath); setDocPreview({ open: true, url, title: doc.title ?? DOCUMENT_TYPE_LABELS[doc.type] }); } catch (e) { alert(e instanceof Error ? e.message : 'Не вдалося відкрити'); } }} className="p-1 text-gray-400 hover:text-white rounded" title="Переглянути"><Eye className="w-4 h-4" /></button><button type="button" onClick={() => { if (window.confirm('Видалити документ безповоротно?')) { const pid = selectedProperty!.id; propertyDocumentsService.deletePropertyDocumentHard(doc).then(() => {}).catch((e) => alert(e?.message || 'Помилка')).finally(() => { propertyDocumentsService.listPropertyDocuments(pid).then(setCard1Documents); }); } }} className="p-1 text-red-400 hover:text-red-300 rounded" title="Видалити"><Trash2 className="w-4 h-4" /></button></div></td></tr>;
                                                        })}
                                                    </tbody>
                                                </table>
                                            </div>
                                        ...
                                        <div className="mb-4">
                                            ...
                                            <div className="overflow-x-auto border border-gray-700 rounded-lg">
                                                <table className="w-full text-xs min-w-[400px] table-fixed">
                                                    <thead className="bg-[#23262b] text-gray-400"><tr><th className="px-2 py-1 text-left font-medium w-24">Datum</th><th className="px-2 py-1 text-left font-medium w-16">Nr</th><th className="px-2 py-1 text-left font-medium min-w-[80px]">Firma</th><th className="px-2 py-1 text-left font-medium min-w-[80px]">Owner</th><th className="px-2 py-1 text-left font-medium w-12">1/2</th><th className="px-2 py-1 w-20" /></tr></thead>
                                                    <tbody className="divide-y divide-gray-700/50">
                                                        {showAddDocumentForm && newDocType === 'zvu' && (
                                                            <tr className="bg-[#0D1117]/80">
                                                                <td className="px-2 py-1"><input type="date" value={String(newDocMeta.datum ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, datum: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td>
                                                                <td className="px-2 py-1"><input value={String(newDocMeta.nr ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, nr: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td>
                                                                <td className="px-2 py-1"><select value={String(newDocMeta.firmaId ?? '')} onChange={e => { const id = e.target.value; const entry = addressBookEntries.find(x => x.id === id); setNewDocMeta(m => ({ ...m, firmaId: id, firmaName: entry?.name ?? '' })); }} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white"><option value="">—</option>{addressBookEntries.map(e => <option key={e.id ?? e.name} value={e.id ?? ''}>{e.name}</option>)}</select></td>
                                                                <td className="px-2 py-1"><select value={String(newDocMeta.ownerId ?? '')} onChange={e => { const id = e.target.value; const entry = addressBookEntries.find(x => x.id === id); setNewDocMeta(m => ({ ...m, ownerId: id, ownerName: entry?.name ?? '' })); }} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white"><option value="">—</option>{addressBookEntries.map(e => <option key={e.id ?? e.name} value={e.id ?? ''}>{e.name}</option>)}</select></td>
                                                                <td className="px-2 py-1"><select value={String(newDocMeta.party ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, party: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white"><option value="">—</option><option value="1">1</option><option value="2">2</option></select></td>
                                                        ...
                                                        {showAddDocumentForm && newDocType === 'zvu' && addDocumentError && <tr><td colSpan={6} className="px-2 py-0.5 text-red-400 text-xs">{addDocumentError}</td></tr>}
                                                        {card1Documents.filter(d => d.type === 'zvu').length === 0 && !(showAddDocumentForm && newDocType === 'zvu') ? <tr><td colSpan={6} className="px-2 py-1 text-gray-500 text-xs whitespace-nowrap">Keine Einträge</td></tr> : card1Documents.filter(d => d.type === 'zvu').map((doc) => {
                                                            const m = (doc.meta || {}) as Record<string, unknown>;
                                                            ...
                                                            return <tr key={doc.id} className="hover:bg-[#1C1F24]"><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.datum ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.nr ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{firmaName}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{ownerName}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.party ?? '—')}</td><td className="px-2 py-1"><div className="flex items-center gap-0.5 justify-end"><button type="button" onClick={async () => { try { const url = await propertyDocumentsService.getDocumentSignedUrl(doc.filePath); setDocPreview({ open: true, url, title: doc.title ?? DOCUMENT_TYPE_LABELS[doc.type] }); } catch (e) { alert(e instanceof Error ? e.message : 'Не вдалося відкрити'); } }} className="p-1 text-gray-400 hover:text-white rounded" title="Переглянути"><Eye className="w-4 h-4" /></button><button type="button" onClick={() => { if (window.confirm('Видалити документ безповоротно?')) { const pid = selectedProperty!.id; propertyDocumentsService.deletePropertyDocumentHard(doc).then(() => {}).catch((e) => alert(e?.message || 'Помилка')).finally(() => { propertyDocumentsService.listPropertyDocuments(pid).then(setCard1Documents); }); } }} className="p-1 text-red-400 hover:text-red-300 rounded" title="Видалити"><Trash2 className="w-4 h-4" /></button></div></td></tr>;
                                                        })}
                                                    </tbody>
                                                </table>
                                            </div>
                                        ...
                                        <div className="mb-4">
                                            ...
                                            <div className="overflow-x-auto border border-gray-700 rounded-lg">
                                                <table className="w-full text-xs min-w-[420px] table-fixed">
                                                    <thead className="bg-[#23262b] text-gray-400"><tr><th className="px-2 py-1 text-left font-medium w-24">Von</th><th className="px-2 py-1 text-left font-medium w-24">Bis</th><th className="px-2 py-1 text-left font-medium min-w-[80px]">Name</th><th className="px-2 py-1 text-left font-medium min-w-[80px]">Vertreter</th><th className="px-2 py-1 w-20" /></tr></thead>
                                                    <tbody className="divide-y divide-gray-700/50">
                                                        {showAddDocumentForm && newDocType === 'an_abmeldung' && (
                                                            <tr className="bg-[#0D1117]/80">
                                                                <td className="px-2 py-1"><input type="date" value={String(newDocMeta.von ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, von: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td>
                                                                <td className="px-2 py-1"><input type="date" value={String(newDocMeta.bis ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, bis: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td>
                                                                <td className="px-2 py-1"><input value={String(newDocMeta.name ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, name: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" placeholder="Vorname Nachname" /></td>
                                                                <td className="px-2 py-1"><input value={String(newDocMeta.vertreter ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, vertreter: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td>
                                                        ...
                                                        {showAddDocumentForm && newDocType === 'an_abmeldung' && addDocumentError && <tr><td colSpan={5} className="px-2 py-0.5 text-red-400 text-xs">{addDocumentError}</td></tr>}
                                                        {card1Documents.filter(d => d.type === 'an_abmeldung').length === 0 && !(showAddDocumentForm && newDocType === 'an_abmeldung') ? <tr><td colSpan={5} className="px-2 py-1 text-gray-500 text-xs whitespace-nowrap">Keine Einträge</td></tr> : card1Documents.filter(d => d.type === 'an_abmeldung').map((doc) => {
                                                            const m = (doc.meta || {}) as Record<string, unknown>;
                                                            const bisVal = m.bis != null && String(m.bis).trim() !== '' ? String(m.bis) : null;
                                                            return <tr key={doc.id} className="hover:bg-[#1C1F24]"><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.von ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{bisVal ? bisVal : <span className="inline-flex items-center px-1.5 py-0.5 rounded text-xs bg-gray-600 text-gray-300">Aktiv</span>}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.name ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.vertreter ?? '—')}</td><td className="px-2 py-1"><div className="flex items-center gap-0.5 justify-end"><button type="button" onClick={async () => { try { const url = await propertyDocumentsService.getDocumentSignedUrl(doc.filePath); setDocPreview({ open: true, url, title: doc.title ?? DOCUMENT_TYPE_LABELS[doc.type] }); } catch (e) { alert(e instanceof Error ? e.message : 'Не вдалося відкрити'); } }} className="p-1 text-gray-400 hover:text-white rounded" title="Переглянути"><Eye className="w-4 h-4" /></button><button type="button" onClick={() => { if (window.confirm('Видалити документ безповоротно?')) { const pid = selectedProperty!.id; propertyDocumentsService.deletePropertyDocumentHard(doc).then(() => {}).catch((e) => alert(e?.message || 'Помилка')).finally(() => { propertyDocumentsService.listPropertyDocuments(pid).then(setCard1Documents); }); } }} className="p-1 text-red-400 hover:text-red-300 rounded" title="Видалити"><Trash2 className="w-4 h-4" /></button></div></td></tr>;
                                                        })}
                                                    </tbody>
                                                </table>
                                            </div>
                                ...
                                {card1DocumentsLoading ? <p className="text-xs text-gray-500">Завантаження…</p> : card1DocumentsError ? <p className="text-xs text-red-400">{card1DocumentsError}</p> : (
                                    <>
                                        <div className="mb-4">
                                            ...
                                            <div className="overflow-x-auto border border-gray-700 rounded-lg"><table className="w-full text-xs min-w-[360px] table-fixed"><thead className="bg-[#23262b] text-gray-400"><tr><th className="px-2 py-1 text-left font-medium w-24">Von</th><th className="px-2 py-1 text-left font-medium w-24">Bis</th><th className="px-2 py-1 text-left font-medium w-16">Nr</th><th className="px-2 py-1 text-left font-medium w-16">Art</th><th className="px-2 py-1 w-20" /></tr></thead><tbody className="divide-y divide-gray-700/50">{showAddDocumentForm && newDocType === 'lease_contract' && (<tr className="bg-[#0D1117]/80"><td className="px-2 py-1"><input type="date" value={String(newDocMeta.von ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, von: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td><td className="px-2 py-1"><input type="date" value={String(newDocMeta.bis ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, bis: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td><td className="px-2 py-1"><input value={String(newDocMeta.nr ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, nr: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td><td className="px-2 py-1"><select value={String(newDocMeta.art ?? 'BEFR')} onChange={e => setNewDocMeta(m => ({ ...m, art: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white"><option value="BEFR">Befr</option><option value="UNBEFR">Unbefr</option><option value="AUTO">Auto</option></select></td><td className="px-2 py-1"><div className="flex items-center gap-0.5 flex-nowrap"><button type="button" onClick={() => addDocumentFileInputRef.current?.click()} className="p-1 text-gray-400 hover:text-white rounded" title="Файл"><Paperclip className="w-3.5 h-3.5" /></button><span className="truncate max-w-16 text-gray-500" title={newDocFile?.name}>{newDocFile?.name ?? '—'}</span><button type="button" disabled={addingDocument || !newDocFile} onClick={async () => { if (!selectedProperty || !newDocFile) return; const von = String(newDocMeta.von ?? '').trim(); const art = String(newDocMeta.art ?? '').trim(); if (!von || !art) { setAddDocumentError('Von та Art обовʼязкові'); return; } setAddingDocument(true); setAddDocumentError(null); const docId = crypto.randomUUID(); let filePath: string | null = null; try { filePath = await propertyDocumentsService.uploadPropertyDocumentFile(newDocFile, selectedProperty.id, 'lease_contract', docId); const meta = { ...newDocMeta }; await propertyDocumentsService.createPropertyDocument({ id: docId, propertyId: selectedProperty.id, type: 'lease_contract', filePath, title: null, meta }); const list = await propertyDocumentsService.listPropertyDocuments(selectedProperty.id); setCard1Documents(list); setNewDocMeta(getDefaultDocMeta('lease_contract')); setNewDocFile(null); setShowAddDocumentForm(false); if (addDocumentFileInputRef.current) addDocumentFileInputRef.current.value = ''; } catch (e) { if (filePath) propertyDocumentsService.removePropertyDocumentFile(filePath).catch(() => {}); setAddDocumentError(e instanceof Error ? e.message : 'Помилка'); } finally { setAddingDocument(false); } }} className="p-1 text-emerald-500 hover:text-emerald-400 rounded" title="Зберегти"><Check className="w-3.5 h-3.5" /></button><button type="button" onClick={() => { setShowAddDocumentForm(false); setAddDocumentError(null); setNewDocMeta({}); setNewDocFile(null); if (addDocumentFileInputRef.current) addDocumentFileInputRef.current.value = ''; }} className="p-1 text-gray-400 hover:text-white rounded" title="Скасувати"><X className="w-3.5 h-3.5" /></button></div></td></tr>)}{showAddDocumentForm && newDocType === 'lease_contract' && addDocumentError && <tr><td colSpan={5} className="px-2 py-0.5 text-red-400 text-xs">{addDocumentError}</td></tr>}{card1Documents.filter(d => d.type === 'lease_contract').length === 0 && !(showAddDocumentForm && newDocType === 'lease_contract') ? <tr><td colSpan={5} className="px-2 py-1 text-gray-500 text-xs whitespace-nowrap">Keine Einträge</td></tr> : card1Documents.filter(d => d.type === 'lease_contract').map((doc) => { const m = (doc.meta || {}) as Record<string, unknown>; return <tr key={doc.id} className="hover:bg-[#1C1F24]"><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.von ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.bis ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.nr ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{ART_LABELS[String(m.art)] ?? String(m.art ?? '—')}</td><td className="px-2 py-1"><div className="flex items-center gap-0.5 justify-end"><button type="button" onClick={async () => { try { const url = await propertyDocumentsService.getDocumentSignedUrl(doc.filePath); setDocPreview({ open: true, url, title: doc.title ?? DOCUMENT_TYPE_LABELS[doc.type] }); } catch (e) { alert(e instanceof Error ? e.message : 'Не вдалося відкрити'); } }} className="p-1 text-gray-400 hover:text-white rounded" title="Переглянути"><Eye className="w-4 h-4" /></button><button type="button" onClick={() => { if (window.confirm('Видалити документ безповоротно?')) { const pid = selectedProperty!.id; propertyDocumentsService.deletePropertyDocumentHard(doc).then(() => {}).catch((e) => alert(e?.message || 'Помилка')).finally(() => { propertyDocumentsService.listPropertyDocuments(pid).then(setCard1Documents); }); } }} className="p-1 text-red-400 hover:text-red-300 rounded" title="Видалити"><Trash2 className="w-4 h-4" /></button></div></td></tr>; })}</tbody></table></div>
                                        </div>
                                        <div className="mb-4">
                                            ...
                                            <div className="overflow-x-auto border border-gray-700 rounded-lg"><table className="w-full text-xs min-w-[360px] table-fixed"><thead className="bg-[#23262b] text-gray-400"><tr><th className="px-2 py-1 text-left font-medium w-24">Datum</th><th className="px-2 py-1 text-left font-medium w-16">Nr</th><th className="px-2 py-1 text-left font-medium w-24">Von</th><th className="px-2 py-1 text-left font-medium w-16">An</th><th className="px-2 py-1 w-20" /></tr></thead><tbody className="divide-y divide-gray-700/50">{showAddDocumentForm && newDocType === 'handover_protocol' && (<tr className="bg-[#0D1117]/80"><td className="px-2 py-1"><input type="date" value={String(newDocMeta.datum ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, datum: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td><td className="px-2 py-1"><input value={String(newDocMeta.nr ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, nr: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td><td className="px-2 py-1"><select value={String(newDocMeta.vonId ?? '')} onChange={e => { const id = e.target.value; const entry = addressBookEntries.find(x => x.id === id); setNewDocMeta(m => ({ ...m, vonId: id, vonName: entry?.name ?? '' })); }} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white"><option value="">—</option>{addressBookEntries.map(e => <option key={e.id ?? e.name} value={e.id ?? ''}>{e.name}</option>)}</select></td><td className="px-2 py-1"><select value={String(newDocMeta.anId ?? '')} onChange={e => { const id = e.target.value; const entry = addressBookEntries.find(x => x.id === id); setNewDocMeta(m => ({ ...m, anId: id, anName: entry?.name ?? '' })); }} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white"><option value="">—</option>{addressBookEntries.map(e => <option key={e.id ?? e.name} value={e.id ?? ''}>{e.name}</option>)}</select></td><td className="px-2 py-1"><div className="flex items-center gap-0.5 flex-nowrap"><button type="button" onClick={() => addDocumentFileInputRef.current?.click()} className="p-1 text-gray-400 hover:text-white rounded" title="Файл"><Paperclip className="w-3.5 h-3.5" /></button><span className="truncate max-w-16 text-gray-500" title={newDocFile?.name}>{newDocFile?.name ?? '—'}</span><button type="button" disabled={addingDocument || !newDocFile} onClick={async () => { if (!selectedProperty || !newDocFile) return; if (addressBookEntries.length === 0) { setAddDocumentError('Немає контрагентів — додай у Контрагенти'); return; } const datum = String(newDocMeta.datum ?? '').trim(); const vonId = String(newDocMeta.vonId ?? '').trim(); const anId = String(newDocMeta.anId ?? '').trim(); if (!datum || !vonId || !anId) { setAddDocumentError('Datum, Von та An обовʼязкові'); return; } setAddingDocument(true); setAddDocumentError(null); const docId = crypto.randomUUID(); let filePath: string | null = null; try { filePath = await propertyDocumentsService.uploadPropertyDocumentFile(newDocFile, selectedProperty.id, 'handover_protocol', docId); const vonName = String(newDocMeta.vonName ?? ''); const anName = String(newDocMeta.anName ?? ''); const meta = { datum, nr: newDocMeta.nr, vonId, vonName, anId, anName, von: vonName, an: anName }; await propertyDocumentsService.createPropertyDocument({ id: docId, propertyId: selectedProperty.id, type: 'handover_protocol', filePath, title: null, meta }); const list = await propertyDocumentsService.listPropertyDocuments(selectedProperty.id); setCard1Documents(list); setNewDocMeta(getDefaultDocMeta('handover_protocol')); setNewDocFile(null); setShowAddDocumentForm(false); if (addDocumentFileInputRef.current) addDocumentFileInputRef.current.value = ''; } catch (e) { if (filePath) propertyDocumentsService.removePropertyDocumentFile(filePath).catch(() => {}); setAddDocumentError(e instanceof Error ? e.message : 'Помилка'); } finally { setAddingDocument(false); } }} className="p-1 text-emerald-500 hover:text-emerald-400 rounded" title="Зберегти"><Check className="w-3.5 h-3.5" /></button><button type="button" onClick={() => { setShowAddDocumentForm(false); setAddDocumentError(null); setNewDocMeta({}); setNewDocFile(null); if (addDocumentFileInputRef.current) addDocumentFileInputRef.current.value = ''; }} className="p-1 text-gray-400 hover:text-white rounded" title="Скасувати"><X className="w-3.5 h-3.5" /></button></div></td></tr>)}{showAddDocumentForm && newDocType === 'handover_protocol' && addDocumentError && <tr><td colSpan={5} className="px-2 py-0.5 text-red-400 text-xs">{addDocumentError}</td></tr>}{card1Documents.filter(d => d.type === 'handover_protocol').length === 0 && !(showAddDocumentForm && newDocType === 'handover_protocol') ? <tr><td colSpan={5} className="px-2 py-1 text-gray-500 text-xs whitespace-nowrap">Keine Einträge</td></tr> : card1Documents.filter(d => d.type === 'handover_protocol').map((doc) => { const m = (doc.meta || {}) as Record<string, unknown>; const vonDisplay = (m.vonId ? addressBookEntries.find(e => e.id === m.vonId)?.name : null) ?? String(m.vonName ?? m.von ?? '—'); const anDisplay = (m.anId ? addressBookEntries.find(e => e.id === m.anId)?.name : null) ?? String(m.anName ?? m.an ?? '—'); return <tr key={doc.id} className="hover:bg-[#1C1F24]"><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.datum ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.nr ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{vonDisplay}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{anDisplay}</td><td className="px-2 py-1"><div className="flex items-center gap-0.5 justify-end"><button type="button" onClick={async () => { try { const url = await propertyDocumentsService.getDocumentSignedUrl(doc.filePath); setDocPreview({ open: true, url, title: doc.title ?? DOCUMENT_TYPE_LABELS[doc.type] }); } catch (e) { alert(e instanceof Error ? e.message : 'Не вдалося відкрити'); } }} className="p-1 text-gray-400 hover:text-white rounded" title="Переглянути"><Eye className="w-4 h-4" /></button><button type="button" onClick={() => { if (window.confirm('Видалити документ безповоротно?')) { const pid = selectedProperty!.id; propertyDocumentsService.deletePropertyDocumentHard(doc).then(() => {}).catch((e) => alert(e?.message || 'Помилка')).finally(() => { propertyDocumentsService.listPropertyDocuments(pid).then(setCard1Documents); }); } }} className="p-1 text-red-400 hover:text-red-300 rounded" title="Видалити"><Trash2 className="w-4 h-4" /></button></div></td></tr>; })}</tbody></table></div>
                                        </div>
                                        <div className="mb-4">
                                            <div className="flex items-center justify-between py-1"><span className="text-xs font-semibold text-white">Utility</span><button type="button" onClick={() => { setNewDocType('supplier_electricity'); setNewDocMeta(getDefaultDocMeta('supplier_electricity')); setNewDocTitle(''); setNewDocDate(''); setNewDocFile(null); setAddDocumentError(null); addDocumentFileInputRef.current && (addDocumentFileInputRef.current.value = ''); setShowAddDocumentForm(true); }} className="p-1 text-emerald-500 hover:text-emerald-400 hover:bg-emerald-500/20 rounded" title="Додати"><Plus className="w-4 h-4" /></button></div>
                                            <div className="overflow-x-auto border border-gray-700 rounded-lg"><table className="w-full text-xs min-w-[680px] table-fixed"><thead className="bg-[#23262b] text-gray-400"><tr><th className="px-2 py-1 text-left font-medium min-w-[120px] w-28">Kind</th><th className="px-2 py-1 text-left font-medium w-20">Anb</th><th className="px-2 py-1 text-left font-medium w-16">Firma</th><th className="px-2 py-1 text-left font-medium w-14">Nr</th><th className="px-2 py-1 text-left font-medium w-14">Betrag</th><th className="px-2 py-1 text-left font-medium w-20">Fällig</th><th className="px-2 py-1 text-left font-medium w-20">Von</th><th className="px-2 py-1 text-left font-medium w-20">Bis</th><th className="px-2 py-1 text-left font-medium w-16">MaLo</th><th className="px-2 py-1 w-20" /></tr></thead><tbody className="divide-y divide-gray-700/50">{showAddDocumentForm && (newDocType === 'supplier_electricity' || newDocType === 'supplier_gas' || newDocType === 'supplier_water' || newDocType === 'supplier_waste') && (<tr className="bg-[#0D1117]/80"><td className="px-2 py-1"><select value={newDocType} onChange={e => { const t = e.target.value as PropertyDocumentType; setNewDocType(t); setNewDocMeta(getDefaultDocMeta(t)); }} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white">{UTILITY_TYPES.map(t => <option key={t} value={t}>{UTILITY_KIND_LABELS[t]}</option>)}</select></td><td className="px-2 py-1"><input value={String(newDocMeta.anbieter ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, anbieter: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td><td className="px-2 py-1"><select value={String(newDocMeta.firma ?? 'SOTISO')} onChange={e => setNewDocMeta(m => ({ ...m, firma: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white"><option value="SOTISO">Sotiso</option><option value="WONOVO">Wonovo</option><option value="NOWFLATS">NowFlats</option></select></td><td className="px-2 py-1"><input value={String(newDocMeta.vertragsnr ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, vertragsnr: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td><td className="px-2 py-1"><input type="number" step={0.01} value={newDocMeta.betrag != null ? Number(newDocMeta.betrag) : ''} onChange={e => setNewDocMeta(m => ({ ...m, betrag: e.target.value === '' ? 0 : parseFloat(e.target.value) || 0 }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td><td className="px-2 py-1"><input type="date" value={String(newDocMeta.faellig ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, faellig: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td><td className="px-2 py-1"><input type="date" value={String(newDocMeta.von ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, von: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td><td className="px-2 py-1"><input type="date" value={String(newDocMeta.bis ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, bis: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td><td className="px-2 py-1"><input value={String(newDocMeta.malo ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, malo: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td><td className="px-2 py-1"><div className="flex items-center gap-0.5 flex-nowrap"><button type="button" onClick={() => addDocumentFileInputRef.current?.click()} className="p-1 text-gray-400 hover:text-white rounded" title="Файл"><Paperclip className="w-3.5 h-3.5" /></button><span className="truncate max-w-12 text-gray-500" title={newDocFile?.name}>{newDocFile?.name ? '…' : '—'}</span><button type="button" disabled={addingDocument || !newDocFile} onClick={async () => { if (!selectedProperty || !newDocFile) return; const von = String(newDocMeta.von ?? '').trim(); const firma = String(newDocMeta.firma ?? '').trim(); if (!von || !firma) { setAddDocumentError('Von та Firma обовʼязкові'); return; } setAddingDocument(true); setAddDocumentError(null); const docId = crypto.randomUUID(); let filePath: string | null = null; try { filePath = await propertyDocumentsService.uploadPropertyDocumentFile(newDocFile, selectedProperty.id, newDocType, docId); const meta: Record<string, unknown> = { ...newDocMeta }; if (typeof meta.betrag === 'string') meta.betrag = parseFloat(meta.betrag as string) || 0; await propertyDocumentsService.createPropertyDocument({ id: docId, propertyId: selectedProperty.id, type: newDocType, filePath, title: null, meta }); const list = await propertyDocumentsService.listPropertyDocuments(selectedProperty.id); setCard1Documents(list); setNewDocMeta(getDefaultDocMeta(newDocType)); setNewDocFile(null); setShowAddDocumentForm(false); if (addDocumentFileInputRef.current) addDocumentFileInputRef.current.value = ''; } catch (e) { if (filePath) propertyDocumentsService.removePropertyDocumentFile(filePath).catch(() => {}); setAddDocumentError(e instanceof Error ? e.message : 'Помилка'); } finally { setAddingDocument(false); } }} className="p-1 text-emerald-500 hover:text-emerald-400 rounded" title="Зберегти"><Check className="w-3.5 h-3.5" /></button><button type="button" onClick={() => { setShowAddDocumentForm(false); setAddDocumentError(null); setNewDocMeta({}); setNewDocFile(null); if (addDocumentFileInputRef.current) addDocumentFileInputRef.current.value = ''; }} className="p-1 text-gray-400 hover:text-white rounded" title="Скасувати"><X className="w-3.5 h-3.5" /></button></div></td></tr>)}{showAddDocumentForm && (newDocType === 'supplier_electricity' || newDocType === 'supplier_gas' || newDocType === 'supplier_water' || newDocType === 'supplier_waste') && addDocumentError && <tr><td colSpan={10} className="px-2 py-0.5 text-red-400 text-xs">{addDocumentError}</td></tr>}{card1Documents.filter(d => UTILITY_TYPES.includes(d.type)).length === 0 && !(showAddDocumentForm && (newDocType === 'supplier_electricity' || newDocType === 'supplier_gas' || newDocType === 'supplier_water' || newDocType === 'supplier_waste')) ? <tr><td colSpan={10} className="px-2 py-1 text-gray-500 text-xs whitespace-nowrap">Keine Einträge</td></tr> : card1Documents.filter(d => UTILITY_TYPES.includes(d.type)).map((doc) => { const m = (doc.meta || {}) as Record<string, unknown>; return <tr key={doc.id} className="hover:bg-[#1C1F24]"><td className="px-2 py-1 text-white truncate whitespace-nowrap">{UTILITY_KIND_LABELS[doc.type] ?? doc.type}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.anbieter ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{FIRMA_LABELS[String(m.firma)] ?? String(m.firma ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.vertragsnr ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{m.betrag != null ? Number(m.betrag) : '—'}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.faellig ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.von ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.bis ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.malo ?? '—')}</td><td className="px-2 py-1"><div className="flex items-center gap-0.5 justify-end"><button type="button" onClick={async () => { try { const url = await propertyDocumentsService.getDocumentSignedUrl(doc.filePath); setDocPreview({ open: true, url, title: doc.title ?? DOCUMENT_TYPE_LABELS[doc.type] }); } catch (e) { alert(e instanceof Error ? e.message : 'Не вдалося відкрити'); } }} className="p-1 text-gray-400 hover:text-white rounded" title="Переглянути"><Eye className="w-4 h-4" /></button><button type="button" onClick={() => { if (window.confirm('Видалити документ безповоротно?')) { const pid = selectedProperty!.id; propertyDocumentsService.deletePropertyDocumentHard(doc).then(() => {}).catch((e) => alert(e?.message || 'Помилка')).finally(() => { propertyDocumentsService.listPropertyDocuments(pid).then(setCard1Documents); }); } }} className="p-1 text-red-400 hover:text-red-300 rounded" title="Видалити"><Trash2 className="w-4 h-4" /></button></div></td></tr>; })}</tbody></table></div>
                                        </div>
                                        <div className="mb-4">
                                            ...
                                            <div className="overflow-x-auto border border-gray-700 rounded-lg"><table className="w-full text-xs min-w-[320px] table-fixed"><thead className="bg-[#23262b] text-gray-400"><tr><th className="px-2 py-1 text-left font-medium w-24">Doc</th><th className="px-2 py-1 text-left font-medium w-24">Von</th><th className="px-2 py-1 text-left font-medium w-24">Bis</th><th className="px-2 py-1 text-left font-medium w-14">Jahr</th><th className="px-2 py-1 w-20" /></tr></thead><tbody className="divide-y divide-gray-700/50">{showAddDocumentForm && newDocType === 'bk_abrechnung' && (<tr className="bg-[#0D1117]/80"><td className="px-2 py-1"><input type="date" value={String(newDocMeta.docDatum ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, docDatum: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td><td className="px-2 py-1"><input type="date" value={String(newDocMeta.von ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, von: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td><td className="px-2 py-1"><input type="date" value={String(newDocMeta.bis ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, bis: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td><td className="px-2 py-1"><input value={String(newDocMeta.jahr ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, jahr: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td><td className="px-2 py-1"><div className="flex items-center gap-0.5 flex-nowrap"><button type="button" onClick={() => addDocumentFileInputRef.current?.click()} className="p-1 text-gray-400 hover:text-white rounded" title="Файл"><Paperclip className="w-3.5 h-3.5" /></button><span className="truncate max-w-16 text-gray-500" title={newDocFile?.name}>{newDocFile?.name ?? '—'}</span><button type="button" disabled={addingDocument || !newDocFile} onClick={async () => { if (!selectedProperty || !newDocFile) return; const docDatum = String(newDocMeta.docDatum ?? '').trim(); if (!docDatum) { setAddDocumentError('Doc обовʼязковий'); return; } setAddingDocument(true); setAddDocumentError(null); const docId = crypto.randomUUID(); let filePath: string | null = null; try { filePath = await propertyDocumentsService.uploadPropertyDocumentFile(newDocFile, selectedProperty.id, 'bk_abrechnung', docId); const meta = { ...newDocMeta }; await propertyDocumentsService.createPropertyDocument({ id: docId, propertyId: selectedProperty.id, type: 'bk_abrechnung', filePath, title: null, meta }); const list = await propertyDocumentsService.listPropertyDocuments(selectedProperty.id); setCard1Documents(list); setNewDocMeta(getDefaultDocMeta('bk_abrechnung')); setNewDocFile(null); setShowAddDocumentForm(false); if (addDocumentFileInputRef.current) addDocumentFileInputRef.current.value = ''; } catch (e) { if (filePath) propertyDocumentsService.removePropertyDocumentFile(filePath).catch(() => {}); setAddDocumentError(e instanceof Error ? e.message : 'Помилка'); } finally { setAddingDocument(false); } }} className="p-1 text-emerald-500 hover:text-emerald-400 rounded" title="Зберегти"><Check className="w-3.5 h-3.5" /></button><button type="button" onClick={() => { setShowAddDocumentForm(false); setAddDocumentError(null); setNewDocMeta({}); setNewDocFile(null); if (addDocumentFileInputRef.current) addDocumentFileInputRef.current.value = ''; }} className="p-1 text-gray-400 hover:text-white rounded" title="Скасувати"><X className="w-3.5 h-3.5" /></button></div></td></tr>)}{showAddDocumentForm && newDocType === 'bk_abrechnung' && addDocumentError && <tr><td colSpan={5} className="px-2 py-0.5 text-red-400 text-xs">{addDocumentError}</td></tr>}{card1Documents.filter(d => d.type === 'bk_abrechnung').length === 0 && !(showAddDocumentForm && newDocType === 'bk_abrechnung') ? <tr><td colSpan={5} className="px-2 py-1 text-gray-500 text-xs whitespace-nowrap">Keine Einträge</td></tr> : card1Documents.filter(d => d.type === 'bk_abrechnung').map((doc) => { const m = (doc.meta || {}) as Record<string, unknown>; return <tr key={doc.id} className="hover:bg-[#1C1F24]"><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.docDatum ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.von ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.bis ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.jahr ?? '—')}</td><td className="px-2 py-1"><div className="flex items-center gap-0.5 justify-end"><button type="button" onClick={async () => { try { const url = await propertyDocumentsService.getDocumentSignedUrl(doc.filePath); setDocPreview({ open: true, url, title: doc.title ?? DOCUMENT_TYPE_LABELS[doc.type] }); } catch (e) { alert(e instanceof Error ? e.message : 'Не вдалося відкрити'); } }} className="p-1 text-gray-400 hover:text-white rounded" title="Переглянути"><Eye className="w-4 h-4" /></button><button type="button" onClick={() => { if (window.confirm('Видалити документ безповоротно?')) { const pid = selectedProperty!.id; propertyDocumentsService.deletePropertyDocumentHard(doc).then(() => {}).catch((e) => alert(e?.message || 'Помилка')).finally(() => { propertyDocumentsService.listPropertyDocuments(pid).then(setCard1Documents); }); } }} className="p-1 text-red-400 hover:text-red-300 rounded" title="Видалити"><Trash2 className="w-4 h-4" /></button></div></td></tr>; })}</tbody></table></div>
                                        </div>
                                        <div className="mb-4">
                                            ...
                                            <div className="overflow-x-auto border border-gray-700 rounded-lg"><table className="w-full text-xs min-w-[400px] table-fixed"><thead className="bg-[#23262b] text-gray-400"><tr><th className="px-2 py-1 text-left font-medium w-24">Datum</th><th className="px-2 py-1 text-left font-medium w-16">Nr</th><th className="px-2 py-1 text-left font-medium min-w-[80px]">Firma</th><th className="px-2 py-1 text-left font-medium min-w-[80px]">Owner</th><th className="px-2 py-1 text-left font-medium w-12">1/2</th><th className="px-2 py-1 w-20" /></tr></thead><tbody className="divide-y divide-gray-700/50">{showAddDocumentForm && newDocType === 'zvu' && (<tr className="bg-[#0D1117]/80"><td className="px-2 py-1"><input type="date" value={String(newDocMeta.datum ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, datum: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td><td className="px-2 py-1"><input value={String(newDocMeta.nr ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, nr: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td><td className="px-2 py-1"><select value={String(newDocMeta.firmaId ?? '')} onChange={e => { const id = e.target.value; const entry = addressBookEntries.find(x => x.id === id); setNewDocMeta(m => ({ ...m, firmaId: id, firmaName: entry?.name ?? '' })); }} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white"><option value="">—</option>{addressBookEntries.map(e => <option key={e.id ?? e.name} value={e.id ?? ''}>{e.name}</option>)}</select></td><td className="px-2 py-1"><select value={String(newDocMeta.ownerId ?? '')} onChange={e => { const id = e.target.value; const entry = addressBookEntries.find(x => x.id === id); setNewDocMeta(m => ({ ...m, ownerId: id, ownerName: entry?.name ?? '' })); }} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white"><option value="">—</option>{addressBookEntries.map(e => <option key={e.id ?? e.name} value={e.id ?? ''}>{e.name}</option>)}</select></td><td className="px-2 py-1"><select value={String(newDocMeta.party ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, party: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white"><option value="">—</option><option value="1">1</option><option value="2">2</option></select></td><td className="px-2 py-1"><div className="flex items-center gap-0.5 flex-nowrap"><button type="button" onClick={() => addDocumentFileInputRef.current?.click()} className="p-1 text-gray-400 hover:text-white rounded" title="Файл"><Paperclip className="w-3.5 h-3.5" /></button><span className="truncate max-w-16 text-gray-500" title={newDocFile?.name}>{newDocFile?.name ?? '—'}</span><button type="button" disabled={addingDocument || !newDocFile} onClick={async () => { if (!selectedProperty || !newDocFile) return; const datum = String(newDocMeta.datum ?? '').trim(); if (!datum) { setAddDocumentError('Datum обовʼязковий'); return; } setAddingDocument(true); setAddDocumentError(null); const docId = crypto.randomUUID(); let filePath: string | null = null; try { filePath = await propertyDocumentsService.uploadPropertyDocumentFile(newDocFile, selectedProperty.id, 'zvu', docId); const meta = { datum: newDocMeta.datum, nr: newDocMeta.nr, firmaId: newDocMeta.firmaId, firmaName: newDocMeta.firmaName, ownerId: newDocMeta.ownerId, ownerName: newDocMeta.ownerName, party: newDocMeta.party }; await propertyDocumentsService.createPropertyDocument({ id: docId, propertyId: selectedProperty.id, type: 'zvu', filePath, title: null, meta }); const list = await propertyDocumentsService.listPropertyDocuments(selectedProperty.id); setCard1Documents(list); setNewDocMeta(getDefaultDocMeta('zvu')); setNewDocFile(null); setShowAddDocumentForm(false); if (addDocumentFileInputRef.current) addDocumentFileInputRef.current.value = ''; } catch (e) { if (filePath) propertyDocumentsService.removePropertyDocumentFile(filePath).catch(() => {}); setAddDocumentError(e instanceof Error ? e.message : 'Помилка'); } finally { setAddingDocument(false); } }} className="p-1 text-emerald-500 hover:text-emerald-400 rounded" title="Зберегти"><Check className="w-3.5 h-3.5" /></button><button type="button" onClick={() => { setShowAddDocumentForm(false); setAddDocumentError(null); setNewDocMeta({}); setNewDocFile(null); if (addDocumentFileInputRef.current) addDocumentFileInputRef.current.value = ''; }} className="p-1 text-gray-400 hover:text-white rounded" title="Скасувати"><X className="w-3.5 h-3.5" /></button></div></td></tr>)}{showAddDocumentForm && newDocType === 'zvu' && addDocumentError && <tr><td colSpan={6} className="px-2 py-0.5 text-red-400 text-xs">{addDocumentError}</td></tr>}{card1Documents.filter(d => d.type === 'zvu').length === 0 && !(showAddDocumentForm && newDocType === 'zvu') ? <tr><td colSpan={6} className="px-2 py-1 text-gray-500 text-xs whitespace-nowrap">Keine Einträge</td></tr> : card1Documents.filter(d => d.type === 'zvu').map((doc) => { const m = (doc.meta || {}) as Record<string, unknown>; const firmaName = (m.firmaId ? addressBookEntries.find(e => e.id === m.firmaId)?.name : null) ?? String(m.firmaName ?? '—'); const ownerName = (m.ownerId ? addressBookEntries.find(e => e.id === m.ownerId)?.name : null) ?? String(m.ownerName ?? '—'); return <tr key={doc.id} className="hover:bg-[#1C1F24]"><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.datum ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.nr ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{firmaName}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{ownerName}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.party ?? '—')}</td><td className="px-2 py-1"><div className="flex items-center gap-0.5 justify-end"><button type="button" onClick={async () => { try { const url = await propertyDocumentsService.getDocumentSignedUrl(doc.filePath); setDocPreview({ open: true, url, title: doc.title ?? DOCUMENT_TYPE_LABELS[doc.type] }); } catch (e) { alert(e instanceof Error ? e.message : 'Не вдалося відкрити'); } }} className="p-1 text-gray-400 hover:text-white rounded" title="Переглянути"><Eye className="w-4 h-4" /></button><button type="button" onClick={() => { if (window.confirm('Видалити документ безповоротно?')) { const pid = selectedProperty!.id; propertyDocumentsService.deletePropertyDocumentHard(doc).then(() => {}).catch((e) => alert(e?.message || 'Помилка')).finally(() => { propertyDocumentsService.listPropertyDocuments(pid).then(setCard1Documents); }); } }} className="p-1 text-red-400 hover:text-red-300 rounded" title="Видалити"><Trash2 className="w-4 h-4" /></button></div></td></tr>; })}</tbody></table></div>
                                        </div>
                                        {/* An-/Abmeldung */}
                                        <div className="mb-4">
                                            ...
                                            <div className="overflow-x-auto border border-gray-700 rounded-lg"><table className="w-full text-xs min-w-[420px] table-fixed"><thead className="bg-[#23262b] text-gray-400"><tr><th className="px-2 py-1 text-left font-medium w-24">Von</th><th className="px-2 py-1 text-left font-medium w-24">Bis</th><th className="px-2 py-1 text-left font-medium min-w-[80px]">Name</th><th className="px-2 py-1 text-left font-medium min-w-[80px]">Vertreter</th><th className="px-2 py-1 w-20" /></tr></thead><tbody className="divide-y divide-gray-700/50">{showAddDocumentForm && newDocType === 'an_abmeldung' && (<tr className="bg-[#0D1117]/80"><td className="px-2 py-1"><input type="date" value={String(newDocMeta.von ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, von: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td><td className="px-2 py-1"><input type="date" value={String(newDocMeta.bis ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, bis: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td><td className="px-2 py-1"><input value={String(newDocMeta.name ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, name: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" placeholder="Vorname Nachname" /></td><td className="px-2 py-1"><input value={String(newDocMeta.vertreter ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, vertreter: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td><td className="px-2 py-1"><div className="flex items-center gap-0.5 flex-nowrap"><button type="button" onClick={() => addDocumentFileInputRef.current?.click()} className="p-1 text-gray-400 hover:text-white rounded" title="Файл"><Paperclip className="w-3.5 h-3.5" /></button><span className="truncate max-w-16 text-gray-500" title={newDocFile?.name}>{newDocFile?.name ?? '—'}</span><button type="button" disabled={addingDocument || !newDocFile} onClick={async () => { if (!selectedProperty || !newDocFile) return; const von = String(newDocMeta.von ?? '').trim(); const name = String(newDocMeta.name ?? '').trim(); if (!von || !name) { setAddDocumentError('Von та Name обовʼязкові'); return; } setAddingDocument(true); setAddDocumentError(null); const docId = crypto.randomUUID(); let filePath: string | null = null; try { filePath = await propertyDocumentsService.uploadPropertyDocumentFile(newDocFile, selectedProperty.id, 'an_abmeldung', docId); const meta = { von: newDocMeta.von, bis: newDocMeta.bis || undefined, name, vertreter: newDocMeta.vertreter || undefined }; await propertyDocumentsService.createPropertyDocument({ id: docId, propertyId: selectedProperty.id, type: 'an_abmeldung', filePath, title: null, meta }); const list = await propertyDocumentsService.listPropertyDocuments(selectedProperty.id); setCard1Documents(list); setNewDocMeta(getDefaultDocMeta('an_abmeldung')); setNewDocFile(null); setShowAddDocumentForm(false); if (addDocumentFileInputRef.current) addDocumentFileInputRef.current.value = ''; } catch (e) { if (filePath) propertyDocumentsService.removePropertyDocumentFile(filePath).catch(() => {}); setAddDocumentError(e instanceof Error ? e.message : 'Помилка'); } finally { setAddingDocument(false); } }} className="p-1 text-emerald-500 hover:text-emerald-400 rounded" title="Зберегти"><Check className="w-3.5 h-3.5" /></button><button type="button" onClick={() => { setShowAddDocumentForm(false); setAddDocumentError(null); setNewDocMeta({}); setNewDocFile(null); if (addDocumentFileInputRef.current) addDocumentFileInputRef.current.value = ''; }} className="p-1 text-gray-400 hover:text-white rounded" title="Скасувати"><X className="w-3.5 h-3.5" /></button></div></td></tr>)}{showAddDocumentForm && newDocType === 'an_abmeldung' && addDocumentError && <tr><td colSpan={5} className="px-2 py-0.5 text-red-400 text-xs">{addDocumentError}</td></tr>}{card1Documents.filter(d => d.type === 'an_abmeldung').length === 0 && !(showAddDocumentForm && newDocType === 'an_abmeldung') ? <tr><td colSpan={5} className="px-2 py-1 text-gray-500 text-xs whitespace-nowrap">Keine Einträge</td></tr> : card1Documents.filter(d => d.type === 'an_abmeldung').map((doc) => { const m = (doc.meta || {}) as Record<string, unknown>; const bisVal = m.bis != null && String(m.bis).trim() !== '' ? String(m.bis) : null; return <tr key={doc.id} className="hover:bg-[#1C1F24]"><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.von ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{bisVal ? bisVal : <span className="inline-flex items-center px-1.5 py-0.5 rounded text-xs bg-gray-600 text-gray-300">Aktiv</span>}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.name ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.vertreter ?? '—')}</td><td className="px-2 py-1"><div className="flex items-center gap-0.5 justify-end"><button type="button" onClick={async () => { try { const url = await propertyDocumentsService.getDocumentSignedUrl(doc.filePath); setDocPreview({ open: true, url, title: doc.title ?? DOCUMENT_TYPE_LABELS[doc.type] }); } catch (e) { alert(e instanceof Error ? e.message : 'Не вдалося відкрити'); } }} className="p-1 text-gray-400 hover:text-white rounded" title="Переглянути"><Eye className="w-4 h-4" /></button><button type="button" onClick={() => { if (window.confirm('Видалити документ безповоротно?')) { const pid = selectedProperty!.id; propertyDocumentsService.deletePropertyDocumentHard(doc).then(() => {}).catch((e) => alert(e?.message || 'Помилка')).finally(() => { propertyDocumentsService.listPropertyDocuments(pid).then(setCard1Documents); }); } }} className="p-1 text-red-400 hover:text-red-300 rounded" title="Видалити"><Trash2 className="w-4 h-4" /></button></div></td></tr>; })}</tbody></table></div>
                                        </div>
                                    </>
                                )}
                            </div>
