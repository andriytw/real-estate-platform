                    ...
                    {isEditingCard1 && card1Draft ? (
                        <>
                            ...
                            <div className="pb-4 border-b border-gray-700">
                                ...
                                <div className="grid grid-cols-12 gap-4 items-start">
                                    <div className="col-span-6"><label className="text-xs text-gray-500 block mb-1">Імʼя</label><input value={card1Draft.tenant.name} onChange={e => setCard1Draft(d => d ? { ...d, tenant: { ...d.tenant, name: e.target.value } } : null)} className="w-full bg-[#111315] border border-gray-700 rounded p-2 text-sm text-white" /></div>
                                    <div className="col-span-6"><label className="text-xs text-gray-500 block mb-1">IBAN (необовʼязково)</label><input value={card1Draft.tenant.iban ?? ''} onChange={e => setCard1Draft(d => d ? { ...d, tenant: { ...d.tenant, iban: e.target.value } } : null)} className="w-full bg-[#111315] border border-gray-700 rounded p-2 text-sm text-white font-mono" /></div>
                                    ...
                                    <div className="col-span-3"><label className="text-xs text-gray-500 block mb-1">День оплати (1–31)</label><input type="number" min={1} max={31} value={card1Draft.tenant.paymentDayOfMonth ?? ''} onChange={e => setCard1Draft(d => d ? { ...d, tenant: { ...d.tenant, paymentDayOfMonth: e.target.value === '' ? undefined : Math.min(31, Math.max(1, parseInt(e.target.value, 10) || 1)) } } : null)} className="w-full bg-[#111315] border border-gray-700 rounded p-2 text-sm text-white" placeholder="—" /></div>
                                    <div className="col-span-12"><label className="text-xs text-gray-500 block mb-1">Телефони</label>{(card1Draft.tenant.phones ?? ['']).map((ph, i) => (<div key={i} className="grid grid-cols-12 gap-2 items-center mb-1"><div className="col-span-10"><input value={ph} onChange={e => setCard1Draft(d => d ? { ...d, tenant: { ...d.tenant, phones: (d.tenant.phones ?? ['']).map((p, j) => j === i ? e.target.value : p) } } : null)} className="w-full bg-[#111315] border border-gray-700 rounded p-2 text-sm text-white" /></div><div className="col-span-2 flex justify-end"><button type="button" onClick={() => setCard1Draft(d => d ? { ...d, tenant: { ...d.tenant, phones: (d.tenant.phones ?? ['']).filter((_, j) => j !== i) } } : null)} className="text-red-400 hover:text-red-300 text-sm">Видалити</button></div></div>))}<button type="button" onClick={() => setCard1Draft(d => d ? { ...d, tenant: { ...d.tenant, phones: [...(d.tenant.phones ?? ['']), ''] } } : null)} className="text-sm text-emerald-500 hover:text-emerald-400">+ Додати телефон</button></div>
                                    <div className="col-span-12"><label className="text-xs text-gray-500 block mb-1">Email</label>{(card1Draft.tenant.emails ?? ['']).map((em, i) => (<div key={i} className="grid grid-cols-12 gap-2 items-center mb-1"><div className="col-span-10"><input type="email" value={em} onChange={e => setCard1Draft(d => d ? { ...d, tenant: { ...d.tenant, emails: (d.tenant.emails ?? []).map((x, j) => j === i ? e.target.value : x) } } : null)} className="w-full bg-[#111315] border border-gray-700 rounded p-2 text-sm text-white" /></div><div className="col-span-2 flex justify-end"><button type="button" onClick={() => setCard1Draft(d => d ? { ...d, tenant: { ...d.tenant, emails: (d.tenant.emails ?? []).filter((_, j) => j !== i) } } : null)} className="text-red-400 hover:text-red-300 text-sm">Видалити</button></div></div>))}<button type="button" onClick={() => setCard1Draft(d => d ? { ...d, tenant: { ...d.tenant, emails: [...(d.tenant.emails ?? ['']), ''] } } : null)} className="text-sm text-emerald-500 hover:text-emerald-400">+ Додати email</button></div>
                                </div>
                            </div>
                            <div className="pb-4 border-b border-gray-700">
                                ...
                                <div className="grid grid-cols-12 gap-4 items-start">
                                    ...
                                    <div className="col-span-12"><label className="text-xs text-gray-500 block mb-1">Телефони</label>{(card1Draft.management?.phones ?? ['']).map((ph, i) => (<div key={i} className="grid grid-cols-12 gap-2 items-center mb-1"><div className="col-span-10"><input value={ph} onChange={e => setCard1Draft(d => d && d.management ? { ...d, management: { ...d.management, phones: (d.management.phones ?? []).map((p, j) => j === i ? e.target.value : p) } } : null)} className="w-full bg-[#111315] border border-gray-700 rounded p-2 text-sm text-white" /></div><div className="col-span-2 flex justify-end"><button type="button" onClick={() => setCard1Draft(d => d && d.management ? { ...d, management: { ...d.management, phones: (d.management.phones ?? []).filter((_, j) => j !== i) } } : null)} className="text-red-400 hover:text-red-300 text-sm">Видалити</button></div></div>))}<button type="button" onClick={() => setCard1Draft(d => d ? { ...d, management: { ...(d.management || defaultContactParty()), phones: [...(d.management?.phones ?? ['']), ''] } } : null)} className="text-sm text-emerald-500 hover:text-emerald-400">+ Додати телефон</button></div>
                                    <div className="col-span-12"><label className="text-xs text-gray-500 block mb-1">Email</label>{(card1Draft.management?.emails ?? ['']).map((em, i) => (<div key={i} className="grid grid-cols-12 gap-2 items-center mb-1"><div className="col-span-10"><input type="email" value={em} onChange={e => setCard1Draft(d => d && d.management ? { ...d, management: { ...d.management, emails: (d.management.emails ?? []).map((x, j) => j === i ? e.target.value : x) } } : null)} className="w-full bg-[#111315] border border-gray-700 rounded p-2 text-sm text-white" /></div><div className="col-span-2 flex justify-end"><button type="button" onClick={() => setCard1Draft(d => d && d.management ? { ...d, management: { ...d.management, emails: (d.management.emails ?? []).filter((_, j) => j !== i) } } : null)} className="text-red-400 hover:text-red-300 text-sm">Видалити</button></div></div>))}<button type="button" onClick={() => setCard1Draft(d => d ? { ...d, management: { ...(d.management || defaultContactParty()), emails: [...(d.management?.emails ?? ['']), ''] } } : null)} className="text-sm text-emerald-500 hover:text-emerald-400">+ Додати email</button></div>
                                </div>
                            </div>
                            <div>
