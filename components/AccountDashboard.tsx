...
const DOC_TABLE = {
  ...
  table: 'w-full table-fixed border-collapse text-xs',
  thead: 'bg-[#1a1d24] border-b border-white/10',
  th: 'px-3 py-3 text-xs font-semibold uppercase tracking-wider text-gray-400 whitespace-nowrap',
  td: 'px-3 py-2.5 text-gray-100 whitespace-nowrap border-b border-white/5 align-middle',
  cellR: 'border-r border-white/5 last:border-r-0',
  row: 'hover:bg-white/[0.04]',
  empty: 'px-3 py-2.5 text-gray-500',
  actions: 'w-[84px]',
};
         ...
         <div className="flex-1 overflow-y-auto p-6 md:p-8 bg-[#0D1117]">
            ...
            <section className="bg-[#1C1F24] p-6 rounded-xl border border-gray-800 shadow-sm mb-6">
                <div className="flex justify-between items-center gap-4 mb-5">
                    <h2 className="text-lg font-semibold text-white">Оренда квартири</h2>
                    {!isEditingCard1 ? (
                ...
                <div className="space-y-4">
                    {isEditingCard1 && card1Draft ? (
                            ...
                            <div>
                                ...
                                <div className="overflow-x-auto rounded-lg border border-white/10">
                                    <table className="w-full text-sm text-left min-w-[800px] border-collapse">
                                        <thead className="bg-[#1a1d24] border-b border-white/10"><tr><th className="px-3 py-3 text-xs font-semibold uppercase tracking-wider text-gray-400" title="Дійсний з">Von</th><th className="px-3 py-3 text-xs font-semibold uppercase tracking-wider text-gray-400" title="Дійсний по">Bis</th><th className="px-3 py-3 text-xs font-semibold uppercase tracking-wider text-gray-400 text-right" title="Kaltmiete">KM</th><th className="px-3 py-3 text-xs font-semibold uppercase tracking-wider text-gray-400 text-right" title="Mietsteuer">MSt</th><th className="px-3 py-3 text-xs font-semibold uppercase tracking-wider text-gray-400 text-right" title="Unternehmenssteuer">USt</th><th className="px-3 py-3 text-xs font-semibold uppercase tracking-wider text-gray-400 text-right" title="Betriebskosten">BK</th><th className="px-3 py-3 text-xs font-semibold uppercase tracking-wider text-gray-400 text-right" title="Heizkosten">HK</th><th className="px-3 py-3 text-xs font-semibold uppercase tracking-wider text-gray-400 text-right" title="Müll">Müll</th><th className="px-3 py-3 text-xs font-semibold uppercase tracking-wider text-gray-400 text-right" title="Strom">Strom</th><th className="px-3 py-3 text-xs font-semibold uppercase tracking-wider text-gray-400 text-right" title="Gas">Gas</th><th className="px-3 py-3 text-xs font-semibold uppercase tracking-wider text-gray-400 text-right" title="Wasser">Wasser</th><th className="px-3 py-3 text-xs font-semibold uppercase tracking-wider text-gray-400 text-right" title="Warmmiete">WM</th></tr></thead>
                                        <tbody className="divide-y divide-white/10 bg-[#16181D]">
                                            {rentTimelineRows.length === 0 ? (
                                              <tr><td colSpan={12} className="py-6 text-center text-gray-500">Немає даних про оренду.</td></tr>
                                              ...
                                              return (
                                                <tr
                                                  ...
                                                  className={isEditing ? '' : 'cursor-pointer hover:bg-white/[0.03]'}
                                                >
                                                  {isEditing && draft ? (
                                                    <>
                                                    ...
                                                    <>
                                                      <td className="px-3 py-2.5 text-white">{r.validFrom}</td><td className="px-3 py-2.5 text-white">{r.validTo}</td><td className="px-3 py-2.5 text-right text-white font-mono">€{(r.km ?? 0).toFixed(2)}</td><td className="px-3 py-2.5 text-right text-white font-mono">€{(r.mietsteuer ?? 0).toFixed(2)}</td><td className="px-3 py-2.5 text-right text-white font-mono">€{(r.unternehmenssteuer ?? 0).toFixed(2)}</td><td className="px-3 py-2.5 text-right text-white font-mono">€{(r.bk ?? 0).toFixed(2)}</td><td className="px-3 py-2.5 text-right text-white font-mono">€{(r.hk ?? 0).toFixed(2)}</td><td className="px-3 py-2.5 text-right text-white font-mono">€{(r.muell ?? 0).toFixed(2)}</td><td className="px-3 py-2.5 text-right text-white font-mono">€{(r.strom ?? 0).toFixed(2)}</td><td className="px-3 py-2.5 text-right text-white font-mono">€{(r.gas ?? 0).toFixed(2)}</td><td className="px-3 py-2.5 text-right text-white font-mono">€{(r.wasser ?? 0).toFixed(2)}</td><td className="px-3 py-2.5 text-right text-emerald-400 font-mono font-bold">€{(r.warm ?? 0).toFixed(2)}</td>
                                                    </>
                                                  )}
                                                </tr>
                                              );
                            ...
                            <div className="pb-4 border-b border-gray-700">
                                ...
                                {card1DocumentsLoading ? <p className="text-xs text-gray-500">Завантаження…</p> : card1DocumentsError ? <p className="text-xs text-red-400">{card1DocumentsError}</p> : (
                                    <>
                                        ...
                                        <div className="mb-4">
                                            <div className="flex items-center justify-between mb-2 border-b border-white/10 pb-2">
                                                <span className="text-sm font-semibold text-white">BKA</span>
                                        ...
                                        <div className="mb-4">
                                            <div className="flex items-center justify-between mb-2 border-b border-white/10 pb-2">
                                                <span className="text-sm font-semibold text-white">ZVU</span>
                                        ...
                                        <div className="mb-4">
                                            <div className="flex items-center justify-between mb-2 border-b border-white/10 pb-2">
                                                <span className="text-sm font-semibold text-white">An-/Abmeldung</span>
                        ...
                        <>
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-4 pb-4 border-b border-gray-700">
                                <div><span className="text-xs text-gray-500 uppercase tracking-wider mb-0.5 block">Адреса</span><span className="text-sm font-medium text-white">{formatPropertyAddress(selectedProperty)}</span></div>
                                <div><span className="text-xs text-gray-500 uppercase tracking-wider mb-0.5 block">Поверх / Сторона</span><span className="text-sm font-medium text-white">{selectedProperty.details?.floor != null ? `${selectedProperty.details.floor} OG` : '—'} {selectedProperty.details?.buildingFloors != null ? ` / ${selectedProperty.details.buildingFloors} поверхов` : ''}</span></div>
                                <div><span className="text-xs text-gray-500 uppercase tracking-wider mb-0.5 block">Квартира / Код</span><span className="text-sm font-medium text-white">{selectedProperty.title || '—'}</span></div>
                            </div>
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-4 pb-4 border-b border-gray-700">
                                <div><span className="text-xs text-gray-500 uppercase tracking-wider mb-0.5 block">Gültig von</span><span className="text-sm font-medium text-white">{leaseTerm?.contract_start || '—'}</span></div>
                                <div><span className="text-xs text-gray-500 uppercase tracking-wider mb-0.5 block">Gültig bis</span><span className="text-sm font-medium text-white">{leaseTerm?.contract_end ?? '—'}</span></div>
                                <div><span className="text-xs text-gray-500 uppercase tracking-wider mb-0.5 block">Vertragstyp</span><span className="text-sm font-medium text-white">{leaseTerm?.contract_type || '—'}</span></div>
                                <div><span className="text-xs text-gray-500 uppercase tracking-wider mb-0.5 block">Erste Mietzahlung ab</span><span className="text-sm font-medium text-white">{leaseTerm?.first_payment_date ?? '—'}</span></div>
                            </div>
                            {(leaseTerm?.note != null && leaseTerm.note.trim() !== '') && (
                                <div className="pb-4 border-b border-gray-700"><span className="text-xs text-gray-500 uppercase tracking-wider mb-0.5 block">Notiz</span><span className="text-sm font-medium text-white">{leaseTerm.note}</span></div>
                            )}
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-4 pb-4 border-b border-gray-700">
                                <div><span className="text-xs text-gray-500 uppercase tracking-wider mb-0.5 block">Статус квартири</span><span className="text-sm font-medium text-white">{selectedProperty.apartmentStatus === 'ooo' ? 'Out of order' : selectedProperty.apartmentStatus === 'preparation' ? 'В підготовці' : selectedProperty.apartmentStatus === 'rented_worker' ? 'Здана працівнику' : 'Активна'}</span></div>
                            </div>
                            <div className="flex items-center justify-between mb-3">
                                ...
                                <div className="flex items-center gap-2">
                                    <button
                                        ...
                                        className="p-2 rounded-lg border border-white/10 bg-white/[0.02] hover:bg-white/[0.04] text-gray-200 flex items-center gap-1.5 text-sm"
                                    >
                                    ...
                                    <button
                                        ...
                                        className="p-2 rounded-lg border border-white/10 bg-white/[0.02] hover:bg-white/[0.04] text-gray-200"
                                        title="Address Book"
                                    >
                                        <BookOpen size={18} />
                                    </button>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 pb-4 border-b border-gray-700">
                                <div className="p-4 rounded-lg border border-white/10 bg-white/[0.02]">
                                    <h3 className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-2">Власник (орендодавець)</h3>
                                ...
                                <div className="p-4 rounded-lg border border-white/10 bg-white/[0.02]">
                                    <h3 className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-2">1-ша фірма</h3>
                                ...
                                <div className="p-4 rounded-lg border border-white/10 bg-white/[0.02]">
                                    <h3 className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-2">2-га фірма</h3>
                                ...
                                <div className="p-4 rounded-lg border border-white/10 bg-white/[0.02]">
                                    <h3 className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-2">Управління</h3>
                            ...
                            <div className="pb-4 border-b border-gray-700">
                                <div className="flex items-center justify-between mb-4">
                                    ...
                                    <button type="button" onClick={() => setShowPaymentDetails(v => !v)} className="p-2 rounded-lg border border-white/10 bg-white/[0.02] hover:bg-white/[0.04] text-gray-200 flex items-center gap-1.5 text-sm">
                                ...
                                {paymentChainLoading ? (
                                    <div className="rounded-lg border border-white/10 bg-white/[0.02] p-4 text-gray-500 text-sm">Завантаження платіжного ланцюжка…</div>
                                ) : (
                            ...
                            <div>
                                ...
                                <div className="overflow-x-auto rounded-lg border border-white/10">
                                    <table className="w-full text-sm text-left min-w-[800px] border-collapse">
                                        <thead className="bg-[#1a1d24] border-b border-white/10"><tr><th className="px-3 py-3 text-xs font-semibold uppercase tracking-wider text-gray-400" title="Дійсний з">Von</th><th className="px-3 py-3 text-xs font-semibold uppercase tracking-wider text-gray-400" title="Дійсний по">Bis</th><th className="px-3 py-3 text-xs font-semibold uppercase tracking-wider text-gray-400 text-right" title="Kaltmiete">KM</th><th className="px-3 py-3 text-xs font-semibold uppercase tracking-wider text-gray-400 text-right" title="Mietsteuer">MSt</th><th className="px-3 py-3 text-xs font-semibold uppercase tracking-wider text-gray-400 text-right" title="Unternehmenssteuer">USt</th><th className="px-3 py-3 text-xs font-semibold uppercase tracking-wider text-gray-400 text-right" title="Betriebskosten">BK</th><th className="px-3 py-3 text-xs font-semibold uppercase tracking-wider text-gray-400 text-right" title="Heizkosten">HK</th><th className="px-3 py-3 text-xs font-semibold uppercase tracking-wider text-gray-400 text-right" title="Müll">Müll</th><th className="px-3 py-3 text-xs font-semibold uppercase tracking-wider text-gray-400 text-right" title="Strom">Strom</th><th className="px-3 py-3 text-xs font-semibold uppercase tracking-wider text-gray-400 text-right" title="Gas">Gas</th><th className="px-3 py-3 text-xs font-semibold uppercase tracking-wider text-gray-400 text-right" title="Wasser">Wasser</th><th className="px-3 py-3 text-xs font-semibold uppercase tracking-wider text-gray-400 text-right" title="Warmmiete">WM</th></tr></thead>
                                        <tbody className="divide-y divide-white/10 bg-[#16181D]">
                                            {rentTimelineRows.length === 0 ? <tr><td colSpan={12} className="py-6 text-center text-gray-500">Немає даних про оренду.</td></tr> : rentTimelineRows.map((r) => <tr key={r.id} className="hover:bg-white/[0.03]"><td className="px-3 py-2.5 text-white">{r.validFrom}</td><td className="px-3 py-2.5 text-white">{r.validTo}</td><td className="px-3 py-2.5 text-right text-white font-mono">€{(r.km ?? 0).toFixed(2)}</td><td className="px-3 py-2.5 text-right text-white font-mono">€{(r.mietsteuer ?? 0).toFixed(2)}</td><td className="px-3 py-2.5 text-right text-white font-mono">€{(r.unternehmenssteuer ?? 0).toFixed(2)}</td><td className="px-3 py-2.5 text-right text-white font-mono">€{(r.bk ?? 0).toFixed(2)}</td><td className="px-3 py-2.5 text-right text-white font-mono">€{(r.hk ?? 0).toFixed(2)}</td><td className="px-3 py-2.5 text-right text-white font-mono">€{(r.muell ?? 0).toFixed(2)}</td><td className="px-3 py-2.5 text-right text-white font-mono">€{(r.strom ?? 0).toFixed(2)}</td><td className="px-3 py-2.5 text-right text-white font-mono">€{(r.gas ?? 0).toFixed(2)}</td><td className="px-3 py-2.5 text-right text-white font-mono">€{(r.wasser ?? 0).toFixed(2)}</td><td className="px-3 py-2.5 text-right text-emerald-400 font-mono font-bold">€{(r.warm ?? 0).toFixed(2)}</td></tr>)}
                                        </tbody>
                                    </table>
                                </div>
                            ...
                            <div className="pb-4 border-b border-gray-700">
                                ...
                                {card1DocumentsLoading ? <p className="text-xs text-gray-500">Завантаження…</p> : card1DocumentsError ? <p className="text-xs text-red-400">{card1DocumentsError}</p> : (
                                    <>
                                        ...
                                        <div className="mb-4">
                                            <div className="flex items-center justify-between mb-2 border-b border-white/10 pb-2"><span className="text-sm font-semibold text-white">Utility</span><button type="button" onClick={() => { setNewDocType('supplier_electricity'); setNewDocMeta(getDefaultDocMeta('supplier_electricity')); setNewDocTitle(''); setNewDocDate(''); setNewDocFile(null); setAddDocumentError(null); addDocumentFileInputRef.current && (addDocumentFileInputRef.current.value = ''); setShowAddDocumentForm(true); }} className="p-1 text-emerald-500 hover:text-emerald-400 hover:bg-emerald-500/20 rounded" title="Додати"><Plus className="w-4 h-4" /></button></div>
