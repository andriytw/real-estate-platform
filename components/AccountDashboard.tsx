...
function getDefaultDocMeta(type: PropertyDocumentType): Record<string, unknown> {
  ...
  if (type === 'handover_protocol') return { datum: '', nr: '', von: '', an: '' };
                                    ...
                                    <>
                                        ...
                                        <div className="mb-4">
                                            <div className="flex items-center justify-between py-1">
                                                ...
                                                <button type="button" onClick={() => { setNewDocType('handover_protocol'); setNewDocMeta(getDefaultDocMeta('handover_protocol')); setNewDocTitle(''); setNewDocDate(''); setNewDocFile(null); setAddDocumentError(null); addDocumentFileInputRef.current && (addDocumentFileInputRef.current.value = ''); setShowAddDocumentForm(true); }} className="p-1 text-emerald-500 hover:text-emerald-400 hover:bg-emerald-500/20 rounded" title="Додати"><Plus className="w-4 h-4" /></button>
                                            </div>
                                            <div className="overflow-x-auto border border-gray-700 rounded-lg">
                                                <table className="w-full text-xs min-w-[360px] table-fixed">
                                                    ...
                                                    <tbody className="divide-y divide-gray-700/50">
                                                        {showAddDocumentForm && newDocType === 'handover_protocol' && (
                                                            <tr className="bg-[#0D1117]/80">
                                                                ...
                                                                <td className="px-2 py-1"><input value={String(newDocMeta.nr ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, nr: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td>
                                                                <td className="px-2 py-1"><input value={String(newDocMeta.von ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, von: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td>
                                                                <td className="px-2 py-1"><input value={String(newDocMeta.an ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, an: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td>
                                                                <td className="px-2 py-1"><div className="flex items-center gap-0.5 flex-nowrap"><button type="button" onClick={() => addDocumentFileInputRef.current?.click()} className="p-1 text-gray-400 hover:text-white rounded" title="Файл"><Paperclip className="w-3.5 h-3.5" /></button><span className="truncate max-w-16 text-gray-500" title={newDocFile?.name}>{newDocFile?.name ?? '—'}</span><button type="button" disabled={addingDocument || !newDocFile} onClick={async () => { if (!selectedProperty || !newDocFile) return; const datum = String(newDocMeta.datum ?? '').trim(); if (!datum) { setAddDocumentError('Datum обовʼязковий'); return; } setAddingDocument(true); setAddDocumentError(null); const docId = crypto.randomUUID(); let filePath: string | null = null; try { filePath = await propertyDocumentsService.uploadPropertyDocumentFile(newDocFile, selectedProperty.id, 'handover_protocol', docId); const meta = { ...newDocMeta }; await propertyDocumentsService.createPropertyDocument({ id: docId, propertyId: selectedProperty.id, type: 'handover_protocol', filePath, title: null, meta }); const list = await propertyDocumentsService.listPropertyDocuments(selectedProperty.id); setCard1Documents(list); setNewDocMeta(getDefaultDocMeta('handover_protocol')); setNewDocFile(null); setShowAddDocumentForm(false); if (addDocumentFileInputRef.current) addDocumentFileInputRef.current.value = ''; } catch (e) { if (filePath) propertyDocumentsService.removePropertyDocumentFile(filePath).catch(() => {}); setAddDocumentError(e instanceof Error ? e.message : 'Помилка'); } finally { setAddingDocument(false); } }} className="p-1 text-emerald-500 hover:text-emerald-400 rounded" title="Зберегти"><Check className="w-3.5 h-3.5" /></button><button type="button" onClick={() => { setShowAddDocumentForm(false); setAddDocumentError(null); setNewDocMeta({}); setNewDocFile(null); if (addDocumentFileInputRef.current) addDocumentFileInputRef.current.value = ''; }} className="p-1 text-gray-400 hover:text-white rounded" title="Скасувати"><X className="w-3.5 h-3.5" /></button></div></td></tr>
                                                        )}
                                                        ...
                                                        {card1Documents.filter(d => d.type === 'handover_protocol').length === 0 && !(showAddDocumentForm && newDocType === 'handover_protocol') ? <tr><td colSpan={5} className="px-2 py-1 text-gray-500 text-xs whitespace-nowrap">Keine Einträge</td></tr> : card1Documents.filter(d => d.type === 'handover_protocol').map((doc) => {
                                                            const m = (doc.meta || {}) as Record<string, unknown>;
                                                            return <tr key={doc.id} className="hover:bg-[#1C1F24]"><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.datum ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.nr ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.von ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.an ?? '—')}</td><td className="px-2 py-1"><div className="flex items-center gap-0.5 justify-end"><button type="button" onClick={async () => { try { const url = await propertyDocumentsService.getDocumentSignedUrl(doc.filePath); setDocPreview({ open: true, url, title: doc.title ?? DOCUMENT_TYPE_LABELS[doc.type] }); } catch (e) { alert(e instanceof Error ? e.message : 'Не вдалося відкрити'); } }} className="p-1 text-gray-400 hover:text-white rounded" title="Переглянути"><Eye className="w-4 h-4" /></button><button type="button" onClick={() => { if (window.confirm('Видалити документ безповоротно?')) { const pid = selectedProperty!.id; propertyDocumentsService.deletePropertyDocumentHard(doc).then(() => {}).catch((e) => alert(e?.message || 'Помилка')).finally(() => { propertyDocumentsService.listPropertyDocuments(pid).then(setCard1Documents); }); } }} className="p-1 text-red-400 hover:text-red-300 rounded" title="Видалити"><Trash2 className="w-4 h-4" /></button></div></td></tr>;
                                                        })}
                                                    </tbody>
                                                </table>
                                            </div>
                            ...
                            <div className="pb-4 border-b border-gray-700">
                                ...
                                {card1DocumentsLoading ? <p className="text-xs text-gray-500">Завантаження…</p> : card1DocumentsError ? <p className="text-xs text-red-400">{card1DocumentsError}</p> : (
                                    <>
                                        ...
                                        <div className="mb-4">
                                            <div className="flex items-center justify-between py-1"><span className="text-xs font-semibold text-white">Übergabeprotokoll</span><button type="button" onClick={() => { setNewDocType('handover_protocol'); setNewDocMeta(getDefaultDocMeta('handover_protocol')); setNewDocTitle(''); setNewDocDate(''); setNewDocFile(null); setAddDocumentError(null); addDocumentFileInputRef.current && (addDocumentFileInputRef.current.value = ''); setShowAddDocumentForm(true); }} className="p-1 text-emerald-500 hover:text-emerald-400 hover:bg-emerald-500/20 rounded" title="Додати"><Plus className="w-4 h-4" /></button></div>
                                            <div className="overflow-x-auto border border-gray-700 rounded-lg"><table className="w-full text-xs min-w-[360px] table-fixed"><thead className="bg-[#23262b] text-gray-400"><tr><th className="px-2 py-1 text-left font-medium w-24">Datum</th><th className="px-2 py-1 text-left font-medium w-16">Nr</th><th className="px-2 py-1 text-left font-medium w-24">Von</th><th className="px-2 py-1 text-left font-medium w-16">An</th><th className="px-2 py-1 w-20" /></tr></thead><tbody className="divide-y divide-gray-700/50">{showAddDocumentForm && newDocType === 'handover_protocol' && (<tr className="bg-[#0D1117]/80"><td className="px-2 py-1"><input type="date" value={String(newDocMeta.datum ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, datum: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td><td className="px-2 py-1"><input value={String(newDocMeta.nr ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, nr: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td><td className="px-2 py-1"><input value={String(newDocMeta.von ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, von: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td><td className="px-2 py-1"><input value={String(newDocMeta.an ?? '')} onChange={e => setNewDocMeta(m => ({ ...m, an: e.target.value }))} className="w-full min-w-0 bg-[#0D1117] border border-gray-700 rounded px-1.5 py-0.5 text-xs text-white" /></td><td className="px-2 py-1"><div className="flex items-center gap-0.5 flex-nowrap"><button type="button" onClick={() => addDocumentFileInputRef.current?.click()} className="p-1 text-gray-400 hover:text-white rounded" title="Файл"><Paperclip className="w-3.5 h-3.5" /></button><span className="truncate max-w-16 text-gray-500" title={newDocFile?.name}>{newDocFile?.name ?? '—'}</span><button type="button" disabled={addingDocument || !newDocFile} onClick={async () => { if (!selectedProperty || !newDocFile) return; const datum = String(newDocMeta.datum ?? '').trim(); if (!datum) { setAddDocumentError('Datum обовʼязковий'); return; } setAddingDocument(true); setAddDocumentError(null); const docId = crypto.randomUUID(); let filePath: string | null = null; try { filePath = await propertyDocumentsService.uploadPropertyDocumentFile(newDocFile, selectedProperty.id, 'handover_protocol', docId); const meta = { ...newDocMeta }; await propertyDocumentsService.createPropertyDocument({ id: docId, propertyId: selectedProperty.id, type: 'handover_protocol', filePath, title: null, meta }); const list = await propertyDocumentsService.listPropertyDocuments(selectedProperty.id); setCard1Documents(list); setNewDocMeta(getDefaultDocMeta('handover_protocol')); setNewDocFile(null); setShowAddDocumentForm(false); if (addDocumentFileInputRef.current) addDocumentFileInputRef.current.value = ''; } catch (e) { if (filePath) propertyDocumentsService.removePropertyDocumentFile(filePath).catch(() => {}); setAddDocumentError(e instanceof Error ? e.message : 'Помилка'); } finally { setAddingDocument(false); } }} className="p-1 text-emerald-500 hover:text-emerald-400 rounded" title="Зберегти"><Check className="w-3.5 h-3.5" /></button><button type="button" onClick={() => { setShowAddDocumentForm(false); setAddDocumentError(null); setNewDocMeta({}); setNewDocFile(null); if (addDocumentFileInputRef.current) addDocumentFileInputRef.current.value = ''; }} className="p-1 text-gray-400 hover:text-white rounded" title="Скасувати"><X className="w-3.5 h-3.5" /></button></div></td></tr>)}{showAddDocumentForm && newDocType === 'handover_protocol' && addDocumentError && <tr><td colSpan={5} className="px-2 py-0.5 text-red-400 text-xs">{addDocumentError}</td></tr>}{card1Documents.filter(d => d.type === 'handover_protocol').length === 0 && !(showAddDocumentForm && newDocType === 'handover_protocol') ? <tr><td colSpan={5} className="px-2 py-1 text-gray-500 text-xs whitespace-nowrap">Keine Einträge</td></tr> : card1Documents.filter(d => d.type === 'handover_protocol').map((doc) => { const m = (doc.meta || {}) as Record<string, unknown>; return <tr key={doc.id} className="hover:bg-[#1C1F24]"><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.datum ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.nr ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.von ?? '—')}</td><td className="px-2 py-1 text-white truncate whitespace-nowrap">{String(m.an ?? '—')}</td><td className="px-2 py-1"><div className="flex items-center gap-0.5 justify-end"><button type="button" onClick={async () => { try { const url = await propertyDocumentsService.getDocumentSignedUrl(doc.filePath); setDocPreview({ open: true, url, title: doc.title ?? DOCUMENT_TYPE_LABELS[doc.type] }); } catch (e) { alert(e instanceof Error ? e.message : 'Не вдалося відкрити'); } }} className="p-1 text-gray-400 hover:text-white rounded" title="Переглянути"><Eye className="w-4 h-4" /></button><button type="button" onClick={() => { if (window.confirm('Видалити документ безповоротно?')) { const pid = selectedProperty!.id; propertyDocumentsService.deletePropertyDocumentHard(doc).then(() => {}).catch((e) => alert(e?.message || 'Помилка')).finally(() => { propertyDocumentsService.listPropertyDocuments(pid).then(setCard1Documents); }); } }} className="p-1 text-red-400 hover:text-red-300 rounded" title="Видалити"><Trash2 className="w-4 h-4" /></button></div></td></tr>; })}</tbody></table></div>
                                        </div>
                                        <div className="mb-4">
