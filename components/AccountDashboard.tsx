                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-start pb-4 border-b border-gray-700">
                                ...
                                <div>
                                    ...
                                    <div className="grid grid-cols-1 gap-2 items-start">
                                        ...
                                        <div><label className="text-xs text-gray-500 block mb-1">Телефони</label>{(card1Draft.tenant.phones ?? ['']).map((ph, i) => (<div key={i} className="grid grid-cols-12 gap-3 items-center mb-1"><div className="col-span-10 min-w-0"><input value={ph} onChange={e => setCard1Draft(d => d ? { ...d, tenant: { ...d.tenant, phones: (d.tenant.phones ?? ['']).map((p, j) => j === i ? e.target.value : p) } } : null)} className="w-full bg-[#111315] border border-gray-700 rounded p-2 text-sm text-white" /></div><div className="col-span-2 flex items-center justify-end gap-1 shrink-0 ml-1"><button type="button" onClick={() => setCard1Draft(d => d ? { ...d, tenant: { ...d.tenant, phones: [...(d.tenant.phones ?? ['']), ''] } } : null)} className="inline-flex items-center justify-center p-2 text-emerald-500 hover:text-emerald-400 hover:bg-emerald-500/20 rounded transition-colors" title="Додати телефон"><Phone className="w-4 h-4" /><Plus className="w-3 h-3 ml-0.5" /></button><button type="button" onClick={() => setCard1Draft(d => d ? { ...d, tenant: { ...d.tenant, phones: (d.tenant.phones ?? ['']).filter((_, j) => j !== i) } } : null)} className="p-2 text-red-400 hover:text-red-300 hover:bg-red-900/30 rounded transition-colors" title="Видалити"><Trash2 className="w-4 h-4" /></button></div></div>))}</div>
                                        <div><label className="text-xs text-gray-500 block mb-1">Email</label>{(card1Draft.tenant.emails ?? ['']).map((em, i) => (<div key={i} className="grid grid-cols-12 gap-3 items-center mb-1"><div className="col-span-10 min-w-0"><input type="email" value={em} onChange={e => setCard1Draft(d => d ? { ...d, tenant: { ...d.tenant, emails: (d.tenant.emails ?? []).map((x, j) => j === i ? e.target.value : x) } } : null)} className="w-full bg-[#111315] border border-gray-700 rounded p-2 text-sm text-white" /></div><div className="col-span-2 flex items-center justify-end gap-1 shrink-0 ml-1"><button type="button" onClick={() => setCard1Draft(d => d ? { ...d, tenant: { ...d.tenant, emails: [...(d.tenant.emails ?? ['']), ''] } } : null)} className="inline-flex items-center justify-center p-2 text-emerald-500 hover:text-emerald-400 hover:bg-emerald-500/20 rounded transition-colors" title="Додати email"><Mail className="w-4 h-4" /><Plus className="w-3 h-3 ml-0.5" /></button><button type="button" onClick={() => setCard1Draft(d => d ? { ...d, tenant: { ...d.tenant, emails: (d.tenant.emails ?? []).filter((_, j) => j !== i) } } : null)} className="p-2 text-red-400 hover:text-red-300 hover:bg-red-900/30 rounded transition-colors" title="Видалити"><Trash2 className="w-4 h-4" /></button></div></div>))}</div>
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div className="grid grid-cols-1 gap-2 items-start">
                                        ...
                                        <div><label className="text-xs text-gray-500 block mb-1">Телефони</label>{(card1Draft.management?.phones ?? ['']).map((ph, i) => (<div key={i} className="grid grid-cols-12 gap-3 items-center mb-1"><div className="col-span-10 min-w-0"><input value={ph} onChange={e => setCard1Draft(d => d && d.management ? { ...d, management: { ...d.management, phones: (d.management.phones ?? []).map((p, j) => j === i ? e.target.value : p) } } : null)} className="w-full bg-[#111315] border border-gray-700 rounded p-2 text-sm text-white" /></div><div className="col-span-2 flex items-center justify-end gap-1 shrink-0 ml-1"><button type="button" onClick={() => setCard1Draft(d => d ? { ...d, management: { ...(d.management || defaultContactParty()), phones: [...(d.management?.phones ?? ['']), ''] } } : null)} className="inline-flex items-center justify-center p-2 text-emerald-500 hover:text-emerald-400 hover:bg-emerald-500/20 rounded transition-colors" title="Додати телефон"><Phone className="w-4 h-4" /><Plus className="w-3 h-3 ml-0.5" /></button><button type="button" onClick={() => setCard1Draft(d => d && d.management ? { ...d, management: { ...d.management, phones: (d.management.phones ?? []).filter((_, j) => j !== i) } } : null)} className="p-2 text-red-400 hover:text-red-300 hover:bg-red-900/30 rounded transition-colors" title="Видалити"><Trash2 className="w-4 h-4" /></button></div></div>))}</div>
                                        <div><label className="text-xs text-gray-500 block mb-1">Email</label>{(card1Draft.management?.emails ?? ['']).map((em, i) => (<div key={i} className="grid grid-cols-12 gap-3 items-center mb-1"><div className="col-span-10 min-w-0"><input type="email" value={em} onChange={e => setCard1Draft(d => d && d.management ? { ...d, management: { ...d.management, emails: (d.management.emails ?? []).map((x, j) => j === i ? e.target.value : x) } } : null)} className="w-full bg-[#111315] border border-gray-700 rounded p-2 text-sm text-white" /></div><div className="col-span-2 flex items-center justify-end gap-1 shrink-0 ml-1"><button type="button" onClick={() => setCard1Draft(d => d ? { ...d, management: { ...(d.management || defaultContactParty()), emails: [...(d.management?.emails ?? ['']), ''] } } : null)} className="inline-flex items-center justify-center p-2 text-emerald-500 hover:text-emerald-400 hover:bg-emerald-500/20 rounded transition-colors" title="Додати email"><Mail className="w-4 h-4" /><Plus className="w-3 h-3 ml-0.5" /></button><button type="button" onClick={() => setCard1Draft(d => d && d.management ? { ...d, management: { ...d.management, emails: (d.management.emails ?? []).filter((_, j) => j !== i) } } : null)} className="p-2 text-red-400 hover:text-red-300 hover:bg-red-900/30 rounded transition-colors" title="Видалити"><Trash2 className="w-4 h-4" /></button></div></div>))}</div>
                                    </div>
                                </div>
                            </div>
                            <div>
